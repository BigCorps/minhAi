...
export const MEMORIAS_MIMES_VIDEO = ['video/mp4', 'video/quicktime', 'video/webm'] as const;

export type MemoriaTipo = 'foto' | 'video';
export type MemoriaStatus = 'reservado' | 'pendente' | 'aprovado' | 'oculto' | 'excluido';
export type MemoriasPacoteStatus = 'nao_contratado' | 'aguardando_pagamento' | 'teste' | 'ativo' | 'expirado';

export function urlMemorias(slug: string) { return `https://${slug}.${DOMINIO}/memorias`; }
export function urlAlbum(slug: string) { return `https://${slug}.${DOMINIO}/album`; }

export function centavosParaBrl(centavos: number) {
  return (centavos / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
}

export function calcularExpiracaoMemorias(dataEvento: string | null | undefined, compradoEm = new Date()) {
