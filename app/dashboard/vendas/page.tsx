    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-white/5 shadow-sm overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 py-3 border-b border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-slate-800/60">
          ...
          <h3 className="font-semibold text-sm text-gray-800 dark:text-gray-100">Impressão Automática no Checkout</h3>
        </div>

        {/* Banner plano inativo */}
        {!hasActivePlan && (
          <div className="mx-5 mt-4 flex items-start gap-2 p-3 rounded-lg border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20">
            <AlertCircle className="w-4 h-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
            <p className="text-xs text-amber-700 dark:text-amber-300">
              <span className="font-semibold">Requer plano mensal ativo.</span>{' '}
              <a href="/dashboard/credits" className="underline font-medium">Contratar Agora →</a>
            </p>
          </div>
        )}
          ...
          {([
            ...
            return (
              <div key={field} className="flex items-center gap-4 px-5 py-4">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                  ativo ? 'bg-orange-100 dark:bg-orange-500/10' : 'bg-gray-100 dark:bg-white/5'
                }`}>
                  <svg className={`w-4 h-4 ${ativo ? 'text-orange-500' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                  </svg>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-sm text-gray-900 dark:text-white">{label}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{descricao}</p>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    ...
                    onClick={() => toggleImpressao(field, ativo)}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none
                      disabled:opacity-40 disabled:cursor-not-allowed ${
                      ativo ? 'bg-orange-500' : 'bg-gray-300 dark:bg-slate-600'
                    }`}
                  >
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${
                      ativo ? 'translate-x-6' : 'translate-x-1'
        ...
        {/* Seletor de tipo de impressora automática */}
        <div className={`px-5 py-4 border-t border-gray-100 dark:border-white/10 ${!hasActivePlan ? 'opacity-50 pointer-events-none' : ''}`}>
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 dark:text-white">Tipo de impressora automática</p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                Define qual impressora será usada nos gatilhos acima.
              </p>
            </div>
            <select
            ...
            >
              <option value="local">🖨️ Local (confirma manualmente)</option>
              <option value="recibo">🧾 Recibo (térmica Bluetooth/USB)</option>
              <option value="remota">📡 Remota (PrintNode — automática)</option>
            </select>
          </div>
        </div>
          ...
          {companyId && (
            <div className="mb-8">
              <ModoToggle
                ...
                initialEnabled={true}
                onToggle={(enabled) => {
                  console.log('Modo Vendas atualizado:', enabled);
                }}
              />
            </div>
          )}
