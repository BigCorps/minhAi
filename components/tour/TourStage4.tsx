'use client'
// components/tour/TourStage4.tsx

import { useState, useEffect, useRef, useCallback } from 'react'
import { STAGE4_SCRIPT, Stage4SceneId } from '@/lib/tour/stage4-script'
...
import SceneCadastro from './scenes/SceneCadastro'
import SceneWizard from './scenes/SceneWizard'
import ScenePublicar from './scenes/ScenePublicar'
import SceneConfig from './scenes/SceneConfig'
import SceneWebApp from './scenes/SceneWebApp'
import SceneIndicacao from './scenes/SceneIndicacao'
import SceneConclusaoZero from './scenes/SceneConclusaoZero'
...
export default function TourStage4({
  initialTheme = 'dark',
  onClose,
  onComplete,
  managerDelay = 2000,
  ...
  // Base: 1x = speakingRate 1.15, 1.5x = 1.30, 2x = 1.50
  ...
  const currentScene = STAGE4_SCRIPT[sceneIndex]
  ...
  const runScene = useCallback(async (index: number) => {
    ...
    const scene = STAGE4_SCRIPT[index]
  ...
  const handleNext = useCallback(() => {
    const isLast = sceneIndex === STAGE4_SCRIPT.length - 1
  ...
  return (
    <div
      ...
      style={{
        ...
        background: isDark ? 'linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)' : '#ffffff',
      ...
      >
        ...
        >
          <div className="w-full h-full transition-opacity ease-in-out" style={{ opacity: sceneVisible ? 1 : 0, transitionDuration: `${FADE_DURATION}ms` }}>
            <SceneRenderer4 id={currentScene.id} isSpeaking={isSpeaking} theme={theme} />
          </div>
        </div>
...
function SceneRenderer4({ id, isSpeaking, theme }: { id: Stage4SceneId; isSpeaking: boolean; theme: 'dark' | 'light' }) {
  switch (id) {
    case 'zeroaoar-cadastro':  return <SceneCadastro />
    case 'zeroaoar-wizard':    return <SceneWizard />
    case 'zeroaoar-publicar':  return <ScenePublicar />
    case 'zeroaoar-config':    return <SceneConfig />
    case 'zeroaoar-webapp':    return <SceneWebApp />
    case 'zeroaoar-indicacao': return <SceneIndicacao />
    case 'zeroaoar-conclusao': return <SceneConclusaoZero />
    default:                   return null
  }
}
