...
// components/consultatec/RestricoesCnpjModal.tsx — mesmo padrão de RestricoesCpfModal, ação restricoes_cnpj.
...
export default function RestricoesCnpjModal({ data, onClose }: Props) {
  ...
  const handleConsultar = async () => {
    ...
    if (!cnpjLimpo || cnpjLimpo.length !== 14 || !validateCNPJ(cnpjLimpo)) {
    ...
    try {
      ...
      const params = pendingParams ?? { company_id: companyId, action: 'restricoes_cnpj', cnpj: cnpjLimpo };

      const { data: res, error: fnError } = await supabase.functions.invoke('ferramentas-consultas', { body: params });
      if (fnError) throw new Error(fnError.message);

      if (res.requires_payment) {
        setPendingParams({ company_id: companyId, action: 'restricoes_cnpj', cnpj: cnpjLimpo, payment_confirmed: true, idempotency_key: res.idempotency_key });
        setStep('input');

        const pixRes = await supabase.functions.invoke('gerar-pix-assistente', {
          body: { company_id: companyId, amount_cents: res.amount_cents, purpose: 'consulta_fee', description: `Restrições CNPJ - R$ ${res.amount_brl}`, brand: 'consultatec' },
        });
        ...
        setPixInfo({ qrCodeUrl: pixRes.data.qr_code_url, pixCode: pixRes.data.pix_code, amountBrl: res.amount_brl ?? '20,00' });
        return;
      }
      ...
      <div className="relative w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl border max-h-[90vh] flex flex-col" style={{ backgroundColor: cor.fundo, borderColor: cor.borda }}>
        <div className="px-6 py-4 border-b flex items-center justify-between flex-shrink-0" style={{ borderColor: cor.borda }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold" style={{ color: cor.tinta }}>Restrições CNPJ</h2>
              <p className="text-sm" style={{ color: cor.tintaMuted }}>Score empresarial e protestos</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {!pixInfo && step === 'input' && (
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: cor.tinta }}>CNPJ *</label>
                <input
                  type="text"
                  value={cnpj}
