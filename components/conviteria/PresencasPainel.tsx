      ...
      {aberto && (
        <div className="mt-4 rounded-2xl border p-4" style={{ backgroundColor: '#fff9fb', borderColor: '#c0607833' }}>
          ...
          {carregando && confirmacoes.length === 0 ? (
            ...
            <p className="rounded-xl bg-white p-4 text-center text-sm" style={{ color: '#7c5560' }}>Nenhuma presença confirmada ainda.</p>
          ) : (
            <ul className="space-y-3">
              {confirmacoes.map((c) => (
                <li key={c.id} className="rounded-xl border bg-white p-3" style={{ borderColor: '#c0607828' }}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="font-semibold" style={{ color: '#40232c' }}>{c.nome}</p>
                      {c.email && <p className="mt-1 flex items-center gap-1.5 break-all text-xs" style={{ color: '#7c5560' }}><Mail className="h-3.5 w-3.5 shrink-0" />{c.email}</p>}
                    </div>
                    <span className="shrink-0 rounded-full px-2 py-1 text-[10px] font-semibold" style={{ backgroundColor: '#fdf0f3', color: '#a04a63' }}>{c.totalPessoas} {c.totalPessoas === 1 ? 'pessoa' : 'pessoas'}</span>
                  </div>

                  {c.acompanhantes.length > 0 && (
                    <div className="mt-3 border-t pt-3" style={{ borderColor: '#c0607818' }}>
                      <p className="mb-1.5 text-[11px] font-semibold" style={{ color: '#7c5560' }}>Família</p>
                      <div className="flex flex-wrap gap-1.5">
                        {c.acompanhantes.map((nome, i) => <span key={`${nome}-${i}`} className="rounded-full px-2 py-1 text-[11px]" style={{ backgroundColor: '#fff4f7', color: '#7c5560' }}>{nome}</span>)}
                      </div>
                    </div>
                  )}

                  <div className="mt-3 flex items-center justify-between gap-2">
                    <span className="text-[10px]" style={{ color: '#9b7b84' }}>{new Date(c.atualizadoEm || c.criadoEm).toLocaleString('pt-BR')}</span>
                    <button type="button" onClick={() => remover(c.id)} className="inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px]" style={{ borderColor: '#e7c5cc', color: '#9b3a4c' }}><Trash2 className="h-3.5 w-3.5" />Remover</button>
                  </div>
                </li>
              ))}
            </ul>
