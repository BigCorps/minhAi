...
const BUCKET_MIDIA = 'conviteria-midia';
...
function normalizarInformacoes(valor: unknown): InformacaoItem[] {
  ...
  return valor
    ...
    .map((x: any, i) => ({
      ...
      imagemUrl: urlHttp(x?.imagemUrl),
    }))
    .filter((x) => x.titulo && x.texto);
}

function assegurarSecao(
  secoes: SecaoConfig[],
  tipo: 'programacao' | 'informacoes',
  ativo: boolean,
) {
  const existente = secoes.find((s) => s.tipo === tipo);
  if (existente) {
    return secoes.map((s) => (s.tipo === tipo ? { ...s, ativo } : s));
  }
  ...
  return [
    ...
    {
      tipo,
      ordem: maior + (tipo === 'programacao' ? 5 : 6),
      ativo,
      config: {},
    },
  ];
...
function caminhoInformacaoDoEvento(url: string | undefined, eventoId: string) {
  if (!url) return null;
  ...
  try {
    ...
    const caminho = pathname.slice(indice + marcador.length);
    const prefixo = `eventos/${eventoId}/informacao-`;
...
export async function GET(req: NextRequest) {
  ...
  if (!eventoId) {
    return NextResponse.json({ erro: 'Convite não informado.' }, { status: 400 });
  }

  const r = await exigirEventoDoUsuario(req, eventoId);
  if ('erro' in r) return NextResponse.json({ erro: r.erro }, { status: r.status });

  const { data: gestao } = await r.admin
    .from('evento_gestao_config')
    .select('rsvp_restrito,qr_modo')
  ...
  return NextResponse.json({
    ...
    gestao: {
      rsvpRestrito: !!gestao?.rsvp_restrito,
      qrModo: gestao?.qr_modo === 'individual' ? 'individual' : 'familia',
    },
  });
}
...
export async function PATCH(req: NextRequest) {
  ...
  const { error: e1 } = await r.admin
    .from('eventos')
    .update({ config: cfgNovo, updated_at: new Date().toISOString() })
    .eq('id', eventoId);

  if (e1) {
    return NextResponse.json({ erro: 'Não foi possível salvar os detalhes.' }, { status: 500 });
  }
  ...
  if (e2) {
    return NextResponse.json(
      { erro: 'Os dados foram salvos, mas as novas seções não foram sincronizadas.' },
      { status: 500 },
    );
  }
  ...
  if (e3) {
    return NextResponse.json(
      { erro: 'Os detalhes foram salvos, mas a configuração do RSVP não.' },
      { status: 500 },
    );
  }

  // Remove do Storage somente imagens deste evento que deixaram de ser usadas.
  // Nunca aceita caminho de outro evento/bucket.
  const atuais = new Set(
    informacoes.map((i) => i.imagemUrl).filter((x): x is string => !!x),
  );
  const remover = (cfgAtual.informacoes ?? [])
    .map((i) => i.imagemUrl)
    .filter((url): url is string => !!url && !atuais.has(url))
    .map((url) => caminhoInformacaoDoEvento(url, eventoId))
    .filter((caminho): caminho is string => !!caminho);

  if (remover.length) {
    await r.admin.storage.from(BUCKET_MIDIA).remove(remover);
  }
