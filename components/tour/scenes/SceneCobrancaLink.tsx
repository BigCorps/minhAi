...
// abre tela de valor → tela de QR Code (Café Exemplo) — em looping
...
// ── Rodapé reutilizável — azul, fonte 2× maior ────────────────────
function Rodape({ small = false }: { small?: boolean }) {
  const fs1 = small ? 'clamp(0.52rem, 1.2vw, 0.72rem)' : 'clamp(0.56rem, 1.3vw, 0.76rem)'
  const fs2 = small ? 'clamp(0.46rem, 1.05vw, 0.64rem)' : 'clamp(0.5rem, 1.15vw, 0.7rem)'
  return (
    <div
      className="flex-shrink-0 mx-3 mb-3 rounded-xl px-3 py-2 text-center"
      style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.25)' }}
    >
      <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: fs1, margin: 0, lineHeight: 1.5 }}>
        Pagamento processado com segurança via Banco Inter e BigCorps
      </p>
      <p style={{ color: 'rgba(96,165,250,0.5)', fontSize: fs2, margin: '2px 0 0' }}>
    ...
    <div className="w-full h-full rounded-2xl overflow-hidden flex flex-col select-none" style={{ background: BG }}>
      ...
      <div className="flex-1 min-h-0 flex gap-2 px-3 py-3">
        ...
        <button
          ...
          style={{ background: 'rgba(0,184,169,0.08)', border: '1.5px solid rgba(0,184,169,0.35)', padding: 'clamp(10px, 2vw, 18px)' }}
        >
          <div style={{ width: 'clamp(28px, 6vw, 44px)', height: 'clamp(28px, 6vw, 44px)' }}><PixIcon /></div>
          <span className="font-bold text-white" style={{ fontSize: 'clamp(0.55rem, 1.3vw, 0.72rem)' }}>Link PIX</span>
        ...
        {/* Card InfinitePay */}
        <div className="flex-1 rounded-xl flex flex-col items-center justify-center gap-2"
          style={{ background: 'rgba(245,158,11,0.06)', border: '1.5px solid rgba(245,158,11,0.25)', padding: 'clamp(10px, 2vw, 18px)' }}>
          <div style={{ width: 'clamp(28px, 6vw, 44px)', height: 'clamp(28px, 6vw, 44px)' }}><InfinitePayIcon /></div>
          <span className="font-bold text-white" style={{ fontSize: 'clamp(0.55rem, 1.3vw, 0.72rem)' }}>Link InfinitePay</span>
          <div className="rounded-lg px-2 py-0.5 font-mono text-center w-full truncate"
...
// ── Tela 2 — Digitar valor ────────────────────────────────────────
function TelaValor({ onConfirm }: { onConfirm: () => void }) {
  ...
  return (
    <div className="w-full h-full rounded-2xl overflow-hidden flex flex-col select-none"
      ...
      <div className="flex flex-col items-center flex-shrink-0 pt-4 pb-2 gap-1">
        ...
        <p className="font-bold text-white" style={{ fontSize: 'clamp(0.6rem, 1.4vw, 0.82rem)', margin: 0 }}>Café Exemplo</p>
        <p style={{ fontSize: 'clamp(0.36rem, 0.85vw, 0.5rem)', color: '#00B8A9', fontWeight: 600, margin: 0 }}>Pagamento via PIX</p>
      </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col items-center justify-center px-4">
        <div className="w-full rounded-2xl p-4 flex flex-col gap-3"
          ...
          <p className="text-white/60" style={{ fontSize: 'clamp(0.4rem, 0.95vw, 0.56rem)', margin: 0 }}>Valor do pagamento</p>
...
// ── Tela 3 — QR Code gerado ───────────────────────────────────────
function TelaQR() {
  const [seconds, setSeconds] = useState(1797)
  ...
  return (
    <div className="w-full h-full rounded-2xl overflow-hidden flex flex-col select-none"
      ...
      <div className="flex flex-col items-center flex-shrink-0 pt-3 pb-1 gap-0.5">
        ...
        <p className="font-bold text-white" style={{ fontSize: 'clamp(0.55rem, 1.3vw, 0.76rem)', margin: 0 }}>Café Exemplo</p>
      ...
      {/* Dois painéis */}
      <div className="flex-1 min-h-0 flex gap-2 px-2 pb-1 pt-1">
        {/* Esquerda — resumo */}
        <div className="flex-1 rounded-xl flex flex-col gap-1.5 p-2"
          ...
          <div className="flex items-center justify-between">
            <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 'clamp(0.28rem, 0.65vw, 0.38rem)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Resumo</span>
      ...
      {/* Rodapé azul — small pois a tela QR tem menos espaço */}
      <Rodape small />
    </div>
  )
}

// ── Orquestrador com loop ─────────────────────────────────────────
...
export default function SceneCobrancaLink() {
  ...
  useEffect(() => {
    // Reset para o início de cada loop
    setScreen('cards')
    setVisible(true)

    // cards → valor (1.2s)
    const t1 = setTimeout(() => transition('valor'), 1200)
    // valor → qr: a TelaValor chama onConfirm após digitar ~1.5s + 0.7s = ~2.2s
    // qr fica visível por 4s antes de resetar
    const t2 = setTimeout(() => transition('qr'), 1200 + 250 + 2200)
    // reset do loop após 4s no QR
    const t3 = setTimeout(() => setLoopKey(k => k + 1), 1200 + 250 + 2200 + 250 + 4000)
  ...
  return (
    <div className="w-full h-full relative" style={{ borderRadius: 16, overflow: 'hidden' }}>
      <div style={{ position: 'absolute', inset: 0, opacity: visible ? 1 : 0, transition: 'opacity 0.25s ease' }}>
        ...
        {screen === 'valor' && <TelaValor onConfirm={() => transition('qr')} />}
        {screen === 'qr'    && <TelaQR />}
      </div>
    </div>
  )
