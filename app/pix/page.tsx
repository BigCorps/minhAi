...
function ThemeToggle({ isDark, onToggle }: { isDark: boolean; onToggle: () => void }) {
  return (
    <button
      ...
      className="fixed top-4 right-4 z-50 w-9 h-9 rounded-full flex items-center justify-center bg-black/8 dark:bg-white/8 border border-black/10 dark:border-white/10 text-black/50 dark:text-white/50 hover:bg-black/12 dark:hover:bg-white/12 transition-colors"
    >
      {isDark ? (
...
function Logos() {
  return (
    <div className="flex items-center justify-center gap-3 mb-6">
      ...
      <span className="text-black/25 dark:text-white/25 text-lg font-light select-none">|</span>
      <Image src="/brands/pix/pixwiki.png" alt="Pix Wiki" width={90} height={36} style={{ width: 'auto', height: 36 }} className="object-contain" />
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-1.5">
        ...
        {optional && (
          <span className="text-[10px] text-black/25 dark:text-white/25">opcional</span>
        )}
      </div>
      {children}
...
function Input({ value, onChange, placeholder, type = 'text' }: {
  ...
  return (
    <input
      ...
      className="w-full px-4 py-2.5 rounded-xl text-sm bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-black dark:text-white placeholder-black/25 dark:placeholder-white/25 focus:outline-none focus:border-green-500/60 transition-colors"
    />
  );
}
...
export default function PixWikiPage() {
  ...
  // ─── Classe de fundo por tema ─────────────────────────────────────────────
  const bg = `${isDark ? 'dark' : ''} min-h-screen bg-white dark:bg-[#0e1117]`;
          ...
          <div className="w-full max-w-sm">
            ...
            <div className="bg-black/[0.03] dark:bg-white/[0.03] rounded-2xl border border-black/8 dark:border-white/8 p-5 flex flex-col gap-3">
              ...
              <form onSubmit={handleEmailSignup} className="flex flex-col gap-2">
                <Input value={authEmail} onChange={setAuthEmail} placeholder="Seu e-mail" type="email" />
                <Input value={authSenha} onChange={setAuthSenha} placeholder="Senha (mín. 6 caracteres)" type="password" />
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-black/[0.03] dark:bg-white/[0.03] rounded-2xl border border-black/8 dark:border-white/8 p-5 sm:p-6 flex flex-col gap-4">
            ...
            <Field label="Chave PIX para receber os saques" hint="CPF, CNPJ, e-mail, telefone ou chave aleatória">
              ...
              />
              {form.chavePixTipo && (
                <p className="text-[10px] text-black/30 dark:text-white/30 -mt-1 capitalize">
          ...
          <div className="flex items-center justify-center gap-2 mt-5 flex-wrap">
            {['Confirmação automática', 'Sem maquininha', 'Zero comprovante falso'].map(t => (
              <span
                key={t}
                className="px-2.5 py-1 rounded-full text-[10px] font-medium bg-black/5 dark:bg-white/5 text-black/40 dark:text-white/40 border border-black/6 dark:border-white/6"
              >
                {t}
              </span>
            ))}
