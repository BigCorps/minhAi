...
const cor = {
  fora: '#ffffff', acento: '#c06078', acentoTexto: '#a04a63',
...
export default function PainelPage() {
  ...
  const carregar = useCallback(async () => {
    ...
    if (!acesso) { router.replace('/convite/entrar'); return; }
      ...
      <div className="mx-auto w-full max-w-2xl flex-1">
        <header className="flex items-center justify-between gap-3 mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={sair} className="flex items-center gap-2 rounded-full border px-3 py-2 text-sm" style={{ borderColor: cor.acento + '44', color: cor.tintaSuave }}><LogOut className="h-4 w-4" /><span className="hidden sm:inline">Sair</span></button>
          </div>
        </header>

        {nome && <p className="mb-6 text-sm" style={{ color: cor.tintaSuave }}>Olá, <strong style={{ color: cor.tinta }}>{nome}</strong></p>}
        <div className="mb-4 flex items-center justify-between">
          <h1 className="text-2xl font-semibold" style={{ color: cor.tinta }}>Meus convites</h1>
          <Link href="/convite/criar" className="flex items-center gap-2 rounded-full px-4 py-2.5 text-sm font-semibold" style={{ backgroundColor: cor.acento, color: cor.blocoTexto }}><Plus className="h-4 w-4" />Novo</Link>
        </div>
