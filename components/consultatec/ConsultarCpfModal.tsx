...
// components/consultatec/ConsultarCpfModal.tsx
// Versão ConsultaTec de "Dados CPF" — porta a lógica do modal original da
// minhAi (mesma chamada a ferramentas-consultas, mesmo fluxo requires_payment),
// mas sem os hooks de voz/Google (não existem nesse produto) e com a tela de
// PIX construída inline no tema creme/preto.

import { useState } from 'react';
import { createPortal } from 'react-dom';
import { X, User, Loader2, AlertCircle, CheckCircle2, Download } from 'lucide-react';
...
export default function ConsultarCpfModal({ data, onClose }: Props) {
    ...
    try {
      ...
      if (res.requires_payment) {
        ...
        const pixRes = await supabase.functions.invoke('gerar-pix-assistente', {
          body: { company_id: companyId, amount_cents: res.amount_cents, purpose: 'consulta_fee', description: `Dados CPF - R$ ${res.amount_brl}` },
        });
        ...
        <div className="px-6 py-4 border-b flex items-center justify-between flex-shrink-0" style={{ borderColor: cor.borda }}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: cor.destaque }}>
              <User className="w-5 h-5" style={{ color: cor.fundo }} />
            </div>
            <div>
              <h2 className="text-lg font-bold" style={{ color: cor.tinta }}>Dados CPF</h2>
              <p className="text-sm" style={{ color: cor.tintaMuted }}>Consulta de dados cadastrais</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {step === 'loading' && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p style={{ color: cor.tinta }}>Consultando CPF...</p>
            </div>
          )}
