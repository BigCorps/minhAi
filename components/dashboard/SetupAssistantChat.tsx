...
// =========================================================
// SetupAssistantChat.tsx v2
// Correções:
// ✅ InputBar e PainelPreview removidos como componentes
//    internos (causavam remount e perda de foco no input)
//    — convertidos para JSX inline
// ✅ isMobile substituído por state com breakpoint manual
//    (useIsMobile retornava true em desktop, abrindo tela cheia)
// =========================================================
...
const CAMPOS_DADOS = [
  ...
  { key: 'brand_description',  pergunta: 'Descreva brevemente a empresa em 1-2 frases.' },
...
export default function SetupAssistantChat({
  ...
  // ✅ Breakpoint manual — evita falso positivo do useIsMobile
  ...
  const avancarParaDados = async () => {
    ...
    if (!camposNec.includes('business_hours') && !dadosEmpresa['business_hours']) camposNec.push('business_hours');
    if (!camposNec.includes('brand_description') && !dadosEmpresa['brand_description']) camposNec.push('brand_description');
    setCamposNecessarios(camposNec);
    const msg = `Perfeito! ${funcoesSelecionadas.size} função(ões) selecionada(s). Agora vou coletar alguns dados.`;
    addAssistantMessage(msg); playTextSafe(msg);
    setEtapa('dados');
  ...
  const perguntarProximoCampo = async (campos: string[], indice: number) => {
    ...
    addAssistantMessage(pergunta); playTextSafe(pergunta);
  };
  ...
  const concluirSetup = async () => {
    ...
    try {
      ...
      const msgFinal = data.success
        ? `Tudo configurado! Seu assistente "${companyName}" está pronto com ${funcoesSelecionadas.size} função(ões) ativa(s). Acesse em eai.app.br/ia/${slug}`
    ...
    <div style={{ overflowY: 'auto', padding: '20px', height: '100%' }}>
      ...
      {etapa === 'concluido' && (
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          ...
          <div style={{ fontSize: '12px', color: C.textMuted, marginBottom: '16px' }}>eai.app.br/ia/{slug}</div>
