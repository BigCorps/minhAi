...
export default function PayRootPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (slug.trim() && parsedValue && !isNaN(parseFloat(parsedValue))) {
      router.push(`/pay/${slug.trim().toLowerCase()}/${parsedValue}`);
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', transition: 'background 0.3s', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ width: '100%', maxWidth: '384px' }}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <div style={{ width: '80px', height: '80px', borderRadius: '16px', background: '#4f46e5', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <rect x="2" y="5" width="20" height="14" rx="2" />
              <line x1="2" y1="10" x2="22" y2="10" />
            </svg>
          </div>
          <h1 style={{ fontSize: '24px', fontWeight: 700, color: textPrimary, margin: '0 0 8px' }}>Pagamento Online</h1>
          <p style={{ color: textSecondary, fontSize: '14px', margin: 0 }}>Digite os dados para pagar</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{
          ...
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, color: textSecondary, marginBottom: '8px' }}>Valor (R$)</label>
            <input
              type="text"
              value={value}
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              fontSize: '16px',
              cursor: (!slug.trim() || !value.trim()) ? 'not-allowed' : 'pointer',
              opacity: (!slug.trim() || !value.trim()) ? 0.5 : 1,
