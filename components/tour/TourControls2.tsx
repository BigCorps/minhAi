...
export default function TourControls2({
  currentIndex,
  total,
  currentId,
  isPlaying,
  ...
  const content = (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        {/* Prev */}
        <button onClick={onPrev} disabled={currentIndex === 0} aria-label="Cena anterior"
          className={`${iconBtn} ${iconBtnColors} disabled:opacity-30 disabled:cursor-not-allowed`}>
        ...
        {/* Wake Lock */}
        {isWakeLockSupported && (
          <button onClick={onToggleWakeLock} aria-label="Wake lock"
  ...
  if (overlayMode) {
    return (
      <div className="absolute bottom-0 left-0 right-0 flex justify-center pb-4 pointer-events-none" style={{ zIndex: 40 }}>
        <div
