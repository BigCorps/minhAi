...
import { emailNovaSolicitacao, gerarTokenRepasse } from '@/lib/conviteria/repasse-email';
...
async function dono(req: NextRequest, eventoId: string) {
  ...
  return { admin, evento: evento as any, conta };
}
...
export async function POST(req: NextRequest) {
  ...
  const saldoAntesCentavos = Number(saldoAntes?.disponivel_centavos ?? 0);

  let {data:recebedor}=await admin.from('recebedores').select('id')
    .eq('evento_id',eventoId).order('created_at').limit(1).maybeSingle();
  ...
  const {data:repasseId,error}=await admin.rpc('solicitar_repasse_evento',{
    p_evento_id:eventoId,p_recebedor_id:recebedor.id,p_valor_centavos:valorCentavos
  });

  if(error){
    if(error.message.includes('saldo_insuficiente'))
      return NextResponse.json({erro:'Saldo insuficiente.'},{status:409});
    if(error.message.includes('repasse_pendente'))
      return NextResponse.json({erro:'Já existe um saque em andamento.'},{status:409});
    return NextResponse.json({erro:'Não foi possível solicitar o saque.'},{status:500});
  }

  const { token, hash } = gerarTokenRepasse();
  const expiraEm = new Date(Date.now() + 72 * 60 * 60 * 1000).toISOString();

  const { error: erroToken } = await admin.rpc('definir_token_repasse', {
  ...
  if (erroToken) {
    console.error('Falha ao definir token do repasse:', erroToken);
    return NextResponse.json({
      ok:true, repasseId,
      aviso:'Saque registrado, mas a notificação operacional falhou. Entre em contato com o suporte.'
    });
  }

  const cfg = (evento.config ?? {}) as any;
  const nomeConvite = cfg.anfitrioes?.exibicao || cfg.evento?.titulo || 'Convite';

  try {
    await emailNovaSolicitacao({
      repasseId,
      token,
      nomeConvite,
      ...
      valorCentavos,
      saldoAntesCentavos,
      saldoDepoisCentavos:Math.max(0, saldoAntesCentavos - valorCentavos),
      solicitadoEm:new Date().toLocaleString('pt-BR',{timeZone:'America/Sao_Paulo'}),
    });
  } catch (e) {
    console.error('Falha ao enviar e-mail operacional do saque:', e);
    return NextResponse.json({
      ok:true, repasseId,
      aviso:'Saque registrado, mas o e-mail operacional não pôde ser enviado. Entre em contato com o suporte.'
    });
  }
  ...
  return NextResponse.json({
    ok:true,
    repasseId,
    mensagem:'Saque solicitado com sucesso. O repasse será realizado via PIX em até 24 horas.'
  });
}
