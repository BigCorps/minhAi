        ...
        {activeTab === 'smart' && (
          <div className="w-full flex flex-col gap-2 sm:gap-3 animate-in fade-in duration-200">
            ...
            <div className={`rounded-xl border px-4 py-2 flex items-center justify-center gap-3 ${
              ...
              <span className={`text-xs font-semibold ${isDark ? 'text-blue-400' : 'text-blue-700'}`}>Comece grátis!</span>
                        ...
                        <div key={pkg.id} className={`relative rounded-xl ${
                          ...
                          <div className="px-4 py-2.5 flex items-center justify-between gap-4">
                            <div>
                              ...
                              <p className="text-base font-bold">R$ {(pkg.price_cents / 100).toFixed(2).replace('.', ',')}<span className="text-xs font-normal opacity-70">/mês</span></p>
                            </div>
                      ...
                      {monthlyPlans.map((pkg) => (
                        <div key={pkg.id} className={`relative rounded-2xl flex flex-col transition-all duration-300 ${
                          ...
                          <div className="p-4 flex flex-col items-center text-center gap-1.5">
                            ...
                            <div><span className="text-xl font-bold">R$ {(pkg.price_cents / 100).toFixed(2).replace('.', ',')}</span><span className="text-xs opacity-70">/mês</span></div>
                ...
                <div className={`[@media(max-height:650px)_and_(max-width:767px)]:hidden`}>
                  <div className={`rounded-xl border px-4 py-2.5 flex items-center justify-between gap-3 flex-wrap ${
                    ...
                    <div className="flex flex-col gap-0.5">
                      ...
                      {['Assistente personalizado', 'Suporte prioritário', 'Treinamento da equipe', 'Integração dedicada'].map(item => (
                        <div key={item} className="flex items-center gap-1.5">
                          <svg className={`w-2.5 h-2.5 flex-shrink-0 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button
                      ...
                      className="py-2 px-4 rounded-xl text-xs font-bold transition-all active:scale-95 bg-blue-600 text-white hover:bg-blue-500 whitespace-nowrap"
                    >
                      Falar com consultor
                    </button>
                  </div>
                      ...
                      <div key={pkg.id} className={`relative rounded-2xl flex flex-col transition-all duration-300 ${
                        ...
                        <div className="p-4 flex flex-col items-center text-center gap-2">
                          ...
                          <div className="space-y-1.5">
                            {[
                              { label: `${pkg.interactions.toLocaleString('pt-BR')} interações`, bolt: true },
                              { label: `R$ ${(pkg.price_per_interaction / 100).toFixed(2).replace('.', ',')} por interação`, bolt: false },
                              { label: 'Pagamento via PIX', bolt: false },
                            ].map(({ label, bolt }) => (
