...
export async function POST(req: NextRequest) {
  ...
  if (arquivo.size > regra.limite) {
    return NextResponse.json(
      { erro: `Arquivo maior que ${Math.max(1, Math.floor(regra.limite / (1024 * 1024)))} MB.` },
      { status: 413 },
    );
  }
  ...
  if (eventoId) {
    ...
    const { data: evento } = await admin
      .from('eventos')
      .select('id, publicado_em, contas!inner(user_id)')
      .eq('id', eventoId)
      .maybeSingle();
    ...
    // Padrinhos e imagens de informações pertencem à Gestão do Evento.
    if ((tipo === 'padrinho' || tipo === 'informacao') && !evento.publicado_em) {
      return NextResponse.json(
  ...
  let conteudo: Buffer | ArrayBuffer = await arquivo.arrayBuffer();
  let contentType = arquivo.type;
  let ext = (regra.ext as Record<string, string>)[arquivo.type] ?? 'bin';

  if (tipo === 'presente' || tipo === 'padrinho' || tipo === 'informacao') {
    try {
      const lado = tipo === 'presente' ? 720 : 1200;
      conteudo = await sharp(Buffer.from(conteudo))
        .rotate()
        .resize({ width: lado, height: lado, fit: 'inside', withoutEnlargement: true })
        .webp({ quality: tipo === 'informacao' ? 80 : 82, effort: 4 })
        .toBuffer();
      contentType = 'image/webp';
      ext = 'webp';
    } catch {
      return NextResponse.json(
        { erro: 'Não foi possível processar esta imagem. Tente outro JPG, PNG, WebP ou HEIC.' },
        { status: 415 },
      );
    }
  }
