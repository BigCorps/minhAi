...
import { aplicarBriefing, ORNAMENTOS_BRIEFING } from '@/lib/conviteria/briefing';
...
const schema = {
  ...
  properties: {
    ...
    local: {
      type: 'object',
      additionalProperties: false,
      required: ['nome', 'logradouro', 'bairro', 'cidade', 'cep'],
      properties: {
    ...
    ornamentoId: {
      anyOf: [
        { type: 'string', enum: [...ORNAMENTOS_BRIEFING] },
        { type: 'null' },
      ],
    },
    recursos: {
      type: 'object',
      additionalProperties: false,
      required: ['rsvp', 'presentes', 'recados', 'foto', 'musica', 'logo'],
      properties: {
...
async function interpretar(texto: string) {
  const tipos = TIPOS_EVENTO.map((t) => `${t.id}: ${t.nome}`).join('; ');
  const temas = TEMAS.map((t) => `${t.id}: ${t.nome}`).join('; ');
  const ornamentos = [
    'floral: delicado/romântico',
    'classico: elegante/tradicional',
    'geometrico: moderno/linhas',
    'minimal: clean/minimalista',
    'festivo: alegre/descontraído',
    'rustico: natural/boho/campo',
  ].join('; ');
...
- temaId e ornamentoId podem ser SUGESTÕES baseadas em adjetivos/cores/estilo,
  mas use null quando não houver sinal suficiente.
- pedidosEspeciais recebe pedidos que o produto não consegue preencher
  automaticamente (animações específicas, desenhos especiais, efeitos etc.).
...
Temas permitidos: ${temas}
Ornamentos permitidos: ${ornamentos}`,
        },
        {
          role: 'user',
          content: texto,
  ...
  if (!r.ok) {
    ...
    console.error('ConviteIA briefing — OpenAI:', r.status, detalhe.slice(0, 300));
