...
export default function Page(){
  ...
  if(key==='fila')return <Wrap t="Fila & Atendimento" s="Configuração da fila e painel de chamadas da sua FuncionarIA."><div className="overflow-hidden rounded-3xl border bg-white"><GerarFilaConfigForm companyId={state.company.id}/></div></Wrap>;
  if(key==='agenda')return <Wrap t="Agenda & Reservas" s="Agenda Google e reservas vinculadas à habilidade contratada, sem depender do plano Smart da minhAi."><FuncionarIAAgendaPanel companyId={state.company.id}/></Wrap>;
  ...
  if(key==='fiscal')return <Wrap t="Notas Fiscais" s="Emissão fiscal usa o provedor Brasil NFE e pode ter custo externo adicional, informado antes da ativação em produção."><FiscalPage/></Wrap>;
  return <Locked/>;
}

function Wrap({t,s,children}:{t:string;s:string;children:React.ReactNode}){
  return <div><div className="text-xs font-black uppercase tracking-[.18em] text-[#6D28D9]">FuncionarIA</div><h1 className="mt-2 text-3xl font-black">{t}</h1><p className="mb-7 mt-2 max-w-3xl text-sm leading-6 text-slate-500">{s}</p>{children}</div>;
}

function Locked(){
  return <div className="mx-auto mt-12 max-w-xl rounded-3xl border border-amber-200 bg-white p-8 text-center"><LockKeyhole className="mx-auto h-9 w-9 text-amber-500"/><h1 className="mt-4 text-2xl font-black">Habilidade não contratada</h1><p className="mt-2 text-sm text-slate-500">Este módulo só aparece quando a habilidade correspondente está ativa.</p><Link href="/dashboard/habilidades" className="mt-6 inline-flex rounded-xl bg-[#6D28D9] px-5 py-3 text-sm font-black text-white">Ver habilidades</Link></div>;
}
