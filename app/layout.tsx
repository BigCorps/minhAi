...
import {
  QrCode, CreditCard, Play, Radio, Search, MapPin, CalendarDays,
  Brain, Wrench, BadgeCheck, FolderOpen, ConciergeBell, ShoppingCart, Camera,
} from 'lucide-react';
...
const FUNCAO_PAGES = [
  {
    ...
    cards: [
      {
        title: 'Contatos', icon: <QrCode />, color: 'blue' as const,
        description: 'Conecte seus clientes aos seus canais em segundos. Exibe QR Codes personalizados para Redes Sociais, Telefones e Site. Facilite o acesso ao WhatsApp, Instagram, ligações diretas e outras redes sociais, aumentando conversão e engajamento.',
      },
      {
        title: 'Financeiro', icon: <CreditCard />, color: 'green' as const,
        description: 'Cobranças rápidas, seguras e automatizadas. Geração de Pix, Débito e Crédito via NFC e Link de Pagamento diretamente pelo assistente com confirmação automática. Simplifique o processo de cobrança e aumente a taxa de pagamento imediato.',
      },
      {
        title: 'Multimídia', icon: <Play />, color: 'blue' as const,
        description: 'Experiência interativa e envolvente. Vídeos, Tutoriais, Publicidades, Playlists, Modo Sequência e Música sob comando. Ideal para totens, recepções, lojas, academias e ambientes corporativos.',
      },
    ],
  },
  {
    ...
    cards: [
      {
        title: 'Informação', icon: <Radio />, color: 'green' as const,
        description: 'Informações atualizadas em tempo real. Apresenta dados sobre o minhAi, sua Marca, Notícias, Feriados Nacionais e tambêm Câmbio. Mantenha clientes e colaboradores sempre informados com respostas instantâneas.',
      },
      {
        title: 'Consultas', icon: <Search />, color: 'blue' as const,
        description: 'Verificações rápidas e confiáveis. Consulta a Dados de CNPJ, CPF, Placa de Veículo, Leilões, Restrições de CPF e CNPJ. Ferramenta ideal para empresas que precisam validar informações antes de fechar negócios.',
      },
      {
        title: 'Localização', icon: <MapPin />, color: 'green' as const,
        description: 'Geolocalização inteligente e precisa. Mostra o Endereço, Busca CEPs, Traça Rota, verifica Trânsito em tempo real e Consulta DDD. Facilite deslocamentos e atendimento logístico.',
      },
      {
        title: 'Agendamento', icon: <CalendarDays />, color: 'blue' as const,
        description: 'Organização automatizada e eficiente. Marcação de Eventos, Consultas, Mostra Agenda e Calendário, Confirma Presença, Reagenda compromissos e até Envia e lê E-mails. Reduza faltas e melhore a gestão do tempo.',
      },
    ],
  },
  {
    ...
    cards: [
      {
        title: 'Conhecimento', icon: <Brain />, color: 'green' as const,
        description: 'Inteligência avançada ao seu alcance. Responde perguntas gerais, gera Orçamentos, Perguntas e Respostas inteligentes, Tradução, Transcrição de Áudio e consulta a Previsão do Tempo. Um verdadeiro centro de inteligência operacional.',
      },
      {
        title: 'Utilitários', icon: <Wrench />, color: 'blue' as const,
        description: 'Ferramentas práticas para o dia a dia. Controla Aparelhos Smart, Cria Lembretes, Cronômetro, Temporizador e Relógio Mundial. Recursos simples que aumentam produtividade e organização.',
      },
      {
        title: 'Identificação', icon: <BadgeCheck />, color: 'green' as const,
        description: 'Controle e segurança automatizados. Reconhecimento Facial, Registro de Ponto, Fila de Atendimento, Geração de Senha e Cadastro de Cliente. Ideal para empresas que precisam organizar fluxo e validar acessos.',
      },
    ],
  },
  {
    ...
    cards: [
      {
        title: 'Arquivos', icon: <FolderOpen />, color: 'blue' as const,
        description: 'Edição e conversão de documentos com IA. Remover Fundo, Duplicar Imagem, Editar Imagem e Converção de Arquivos automaticamente. Agilidade para marketing, administrativo e operacional.',
      },
      {
        title: 'Serviços', icon: <ConciergeBell />, color: 'green' as const,
        description: 'Soluções operacionais completas. Auxilia os clientes a Imprimir documentos, exibe Tabela de Preços, Cardápio (Menu), Emite Recibos e Chamado ao Suporte agilizado. Facilite o atendimento presencial e digital.',
      },
      {
        title: 'Comercial', icon: <ShoppingCart />, color: 'blue' as const,
        description: 'Aumente suas vendas com inteligência. Recomenda e Cadastra Produtos, gera Avaliações, com modo Totem de Vendas, Resposta de Pesquisas e Consulta ao Estoque. Transforme o assistente em um vendedor digital ativo 24h.',
      },
      {
        title: 'Câmera', icon: <Camera />, color: 'green' as const,
        description: 'Leitura e validação instantânea. Leitura de QR Codes e Código de Barras, Envia Arquivos, Validação de Cupom e Verificação de Acesso. Ideal para controle de entradas, promoções e operações rápidas.',
      },
    ],
  },
];
...
export default function LandingPage() {
  ...
  const scrollToSection = useCallback((id: string) => {
    const targetId =
      id === 'funcoes' ? FUNCAO_PAGES[0].id :
      id === 'recursos' ? RECURSO_IMAGE_SLIDES[0].id : id;
  ...
  // className reutilizável para todas as sections
  const sectionClass = "w-screen h-dvh-safe flex-shrink-0 snap-start snap-always overflow-y-auto";
  ...
  return (
    <div className={`relative h-dvh-safe w-screen overflow-hidden transition-colors duration-500 ${
      ...
      >
        ...
        <style jsx global>{`.h-dvh-safe { height: 100vh; height: 100dvh; }`}</style>

        {/* INÍCIO — overflow-y-auto permite scroll vertical se conteúdo cortar */}
        <section id="inicio" className={sectionClass}>
          <InicioSection theme={theme} />
        </section>

        {/* RECURSOS — imagem + texto */}
        {RECURSO_IMAGE_SLIDES.slice(0, 3).map((slide, index) => (
          <section key={slide.id} id={slide.id} className={sectionClass}>
            <RecursoImageSlide
              theme={theme}
              label={slide.label!}
              title={slide.title!}
        ...
        {/* RECURSOS — Extras */}
        <section id="recurso-extras" className={sectionClass}>
          <VantagensSlide theme={theme} currentIndex={3} totalCount={TOTAL_RECURSO_SLIDES} />
        </section>

        {/* RECURSOS — Cards */}
        <section id="recurso-cards" className={sectionClass}>
          <RecursoCardsSlide
            theme={theme}
        ...
        {FUNCAO_PAGES.map((page, index) => (
          <section key={page.id} id={page.id} className={sectionClass}>
            <FuncaoCardsSlide
              theme={theme}
              cards={page.cards}
              currentIndex={index}
        ...
        {/* PREÇOS */}
        <section id="precos" className={sectionClass}>
