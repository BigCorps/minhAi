...
function VendasPageContent() {
  ...
  useEffect(() => {
    ...
    async function checkAssistantType() {
      if (!companyId) return;
      const { data } = await supabase
        .from('companies')
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {companyId && (
            <div className="mb-8">
              {isVendas ? (
                <div className="flex items-center justify-between gap-4 px-4 py-2.5 rounded-xl border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-900/10">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-amber-800 dark:text-amber-200">Modo Vendas</span>
                    <span className="text-xs text-amber-600 dark:text-amber-400">Sempre ativo na versão Vendas</span>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="text-xs font-medium text-amber-600 dark:text-amber-400">Ativo</span>
                    <button
                      disabled
                      className="relative inline-flex h-5 w-9 items-center rounded-full bg-amber-500 opacity-60 cursor-not-allowed"
                    >
