...
export function AssistantProvider({ children }: { children: ReactNode }) {
  ...
  function setSelectedAssistant(id: string | null, name: string | null) {
    ...
    if (id && name) {
      // Uma FuncionarIA recém-criada ainda não existe na lista carregada no
      // mount do provider. Incluí-la aqui deixa o seletor multiempresa coerente
      // imediatamente após o onboarding, sem depender de hard refresh.
      setAvailableAssistants(current => {
        const existing = current.find(item => item.id === id);

        if (existing) {
          if (existing.name === name) return current;
          return current.map(item => item.id === id ? { ...item, name } : item);
        }
