        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder={isTotem ? 'Ex: Bomba 3, Caixa 1' : 'Nome completo'}
              className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-slate-800 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-white/10 flex items-center justify-between sticky bottom-0 bg-white dark:bg-slate-900">
            ...
            <div>
              {confirmDelete ? (
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={handleDeletar} disabled={saving}
          ...
          <div className="flex items-center gap-3 ml-auto">
            ...
            <button onClick={handleSalvar} disabled={saving}
              className="inline-flex items-center gap-2 px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition disabled:opacity-60">
...
// ── ABA: Cadastros (registrations — tabela original) ──────────────────────────
      ...
      {!loading && filtered.length === 0 && (
        <div className="bg-white/50 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10 p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
            {search ? 'Nenhum resultado' : 'Nenhum cadastro ainda'}
          </h3>
        ...
        <div className="bg-white/50 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden">
                  ...
                  <tr key={r.id} className="border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition">
                    ...
                    {allFieldKeys.map(k => (
                      <td key={k} className="px-4 py-3 text-gray-900 dark:text-white max-w-[200px] truncate">
                        {r.fields?.[k] ?? <span className="text-gray-300 dark:text-gray-600">—</span>}
                      </td>
                    ))}
          ...
          <div className="px-4 py-3 border-t border-gray-100 dark:border-white/5 flex items-center justify-between">
            <p className="text-xs text-gray-400 dark:text-gray-500">
              {filtered.length} de {registrations.length} cadastro{registrations.length !== 1 ? 's' : ''}
            </p>
...
function AbaClientes({ companyId }: { companyId: string }) {
  ...
  const filtered = profiles.filter(p => {
    if (!search.trim()) return true;
    return `${p.nome} ${p.email ?? ''} ${p.identificador ?? ''}`.toLowerCase().includes(search.toLowerCase());
  }).sort((a, b) => {
    const diff = new Date(a.created_at).getTime() - new Date(b.created_at).getTime();
    return sortOrder === 'desc' ? -diff : diff;
  });
  ...
  function exportCSV() {
    ...
    const rows = filtered.map(p => [
      ...
      p.nome, p.email ?? '', p.identificador ?? '',
      p.endereco ?? '',
    ...
    <div className="space-y-4">
      ...
      {!loading && filtered.length === 0 && (
        <div className="bg-white/50 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10 p-12 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            Clientes aparecem aqui após se cadastrarem via função "Minha Conta" no slug.
          </p>
        </div>
      )}
              ...
              <tbody>
                {filtered.map(p => (
                  <tr key={p.id} className="border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition">
                    ...
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{p.email ?? <span className="text-gray-300 dark:text-gray-600">—</span>}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{p.identificador ?? <span className="text-gray-300 dark:text-gray-600">—</span>}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400 whitespace-nowrap">
                      {p.ultimo_acesso ? formatDate(p.ultimo_acesso) : <span className="text-gray-300 dark:text-gray-600">—</span>}
                    </td>
                    <td className="px-4 py-3">
    ...
    <div className="space-y-4">
      ...
      {!loading && filtered.length === 0 && (
        <div className="bg-white/50 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10 p-12 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mb-4">
            Cadastre frentistas, atendentes, caixas e gerentes aqui.
          </p>
          {!search && !tipoFiltro && (
            <button onClick={() => { setEditando(null); setModalAberto(true); }}
              ...
              <tbody>
                {filtered.map(p => (
                  <tr key={p.id} className="border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition">
                    ...
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{p.email ?? <span className="text-gray-300 dark:text-gray-600">—</span>}</td>
                    <td className="px-4 py-3">
                      {p.pin ? <span className="font-mono text-gray-400">••••</span> : <span className="text-gray-300 dark:text-gray-600">—</span>}
                    </td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400 whitespace-nowrap">
                      {p.ultimo_acesso ? formatDate(p.ultimo_acesso) : <span className="text-gray-300 dark:text-gray-600">—</span>}
                    </td>
      ...
      {modalAberto && (
        <ProfileModal
          companyId={companyId}
          profile={editando}
          tiposDisponiveis={TIPOS_COLABORADOR}
    ...
    <div className="space-y-4">
      ...
      {!loading && profiles.length === 0 && (
        <div className="bg-white/50 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10 p-12 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mb-4">
            Cadastre totens para identificar dispositivos fixos como bombas de posto, caixas de restaurante, etc.
          </p>
          <button onClick={() => { setEditando(null); setModalAberto(true); }}
            ...
            <div key={p.id}
              ...
              {p.ultimo_acesso && (
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">
                  Último acesso: {formatDate(p.ultimo_acesso)}
                </p>
              )}
            </div>
          ))}
              ...
              <div className="grid grid-cols-4 sm:flex border-b border-gray-200 dark:border-white/10">
                {abas.map(({ key, label, icon: Icon }) => (
                  <button
                    key={key}
                    onClick={() => setAba(key)}
                    className={`sm:flex-1 px-3 py-3 text-sm font-medium transition flex items-center justify-center gap-2
