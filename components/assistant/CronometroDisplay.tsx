...
// Caminho: components/assistant/CronometroDisplay.tsx
//
// Comportamento:
// - Inicia automaticamente ao abrir
// - Só para quando o assistente detectar "finalizar cronômetro"
//   (o VoiceCommandDetector chama onClose via stopEverything)
// - Ao parar: pisca o tempo final por 15 segundos e fecha
// - Fecha no X, em 15s após parar, ou por comando de voz
// ============================================================
...
interface Props {
  data: {
    companyId: string;
    autoStop?: boolean; // se true, para imediatamente (para testes)
  };
  onClose: () => void;
  theme: 'dark' | 'light';
}
...
export default function CronometroDisplay({ data, onClose, theme }: Props) {
  ...
  // ── Expõe método de parar para o VoiceAssistant ───────────
  // O assistente detecta "finalizar cronômetro" → chama handleStop
  // via window event (escutamos aqui)
  useEffect(() => {
      ...
      <div className={`relative w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden
        ...
        <div className="p-8 text-center">
          ...
          <div className={`font-mono text-5xl font-bold mb-2 transition-all
            ${blinking ? 'animate-pulse text-blue-400' : isDark ? 'text-white' : 'text-gray-900'}
            ${stopped ? 'scale-110' : ''}`}
