...
// Mock do QRCodeDisplay — cicla entre WhatsApp, PIX e Instagram
...
// QR Code SVG gerado estaticamente (padrão visual, não funcional)
function MockQRSVG({ color }: { color: string }) {
  return (
    <svg viewBox="0 0 100 100" className="w-full h-full">
      {/* Canto superior esquerdo */}
      <rect x="5" y="5" width="30" height="30" rx="3" fill="none" stroke={color} strokeWidth="4"/>
      <rect x="12" y="12" width="16" height="16" rx="1" fill={color}/>
      {/* Canto superior direito */}
      <rect x="65" y="5" width="30" height="30" rx="3" fill="none" stroke={color} strokeWidth="4"/>
      <rect x="72" y="12" width="16" height="16" rx="1" fill={color}/>
      {/* Canto inferior esquerdo */}
      <rect x="5" y="65" width="30" height="30" rx="3" fill="none" stroke={color} strokeWidth="4"/>
      <rect x="12" y="72" width="16" height="16" rx="1" fill={color}/>
      {/* Módulos centrais */}
      {[
        [42,5],[50,5],[58,5],[42,13],[58,13],[42,21],[50,21],
        [5,42],[13,42],[21,42],[5,50],[21,50],[5,58],[13,58],
        [42,42],[50,42],[58,42],[66,42],[74,42],[82,42],[90,42],
        [42,50],[66,50],[82,50],[90,50],
        [42,58],[50,58],[66,58],[74,58],[90,58],
        [42,66],[58,66],[66,66],[82,66],
        [42,74],[50,74],[58,74],[74,74],[82,74],[90,74],
        [42,82],[66,82],[74,82],[90,82],
        [42,90],[50,90],[58,90],[66,90],[82,90],
      ].map(([x, y], i) => (
        <rect key={i} x={x} y={y} width="6" height="6" fill={color} />
      ))}
    </svg>
  )
}
...
export default function SceneQRCode() {
  ...
  return (
    <div
      ...
      style={{ background: BG }}
    >
      {/* Card do QR Code — replica QRCodeDisplay */}
      <div
      ...
      >
        ...
        {/* QR Code */}
        <div className="relative bg-white p-3" style={{ aspectRatio: '1/1' }}>
          <MockQRSVG color={data.color} />
          {/* Logo central */}
          <div
          ...
          >
            <div
              ...
              style={{
                width: 'clamp(22px, 8%, 32px)',
                height: 'clamp(22px, 8%, 32px)',
                background: data.color,
                color: 'white',
              }}
            >
              <span style={{ fontSize: 'clamp(0.5rem, 1.5vw, 0.7rem)' }}>{data.icon}</span>
            </div>
          </div>
        </div>
