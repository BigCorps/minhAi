// =================================================================================
// ARQUIVO: components/modals/ConsultarPlacaModal.tsx
...
const OPENING_TEXT = "Informe a placa do veículo que deseja consultar";
...
export function ConsultarPlacaModal({
  isOpen,
  onClose,
  companyId,
  playText,
  ...
  const handleConsultar = async () => {
    ...
    try {
      ...
      if (execData.status === 'PAGO_AUTOMATICAMENTE' || execData.status === 'EXECUTADO_GRATUITAMENTE') {
        ...
        playText('Consulta realizada com sucesso. Dados do veículo exibidos.');
      }
    } catch (err: any) {
      console.error('Erro ao consultar placa:', err);
      setError(err.message || 'Erro ao consultar placa.');
      playText('Erro ao consultar placa. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPdf = () => {
    ...
    link.href = pdfBase64;
    link.download = `consulta_placa_${placa.replace(/[^A-Z0-9]/gi, '')}.pdf`;
    link.click();
  };
