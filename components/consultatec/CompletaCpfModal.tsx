...
// components/consultatec/CompletaCpfModal.tsx
//
// Só existe pra CPF (Dados + Restrições + Protestos). Chama a ação nova
// 'completa_cpf' em ferramentas-consultas (ver patch enviado) — o resto do
// fluxo (saldo insuficiente → PIX → confirmação) é idêntico aos outros
// modais, porque a edge function trata 'completa_cpf' com o mesmo
// mecanismo de requires_payment que as demais ações.

import { useState } from 'react';
import { createPortal } from 'react-dom';
import { X, FileStack, Loader2, AlertCircle, CheckCircle2, Download } from 'lucide-react';
...
export default function CompletaCpfModal({ data, onClose }: CompletaCpfModalProps) {
  ...
  const handleConsultar = async () => {
    ...
    try {
      ...
      const params = pendingParams ?? { company_id: companyId, action: 'completa_cpf', cpf: cpfLimpo };

      const { data: res, error: fnError } = await supabase.functions.invoke('ferramentas-consultas', {
        body: params,
      });
      ...
      if (res.requires_payment) {
        ...
        setPixInfo({
          ...
          amountBrl: res.amount_brl ?? '28,00',
        });
        return;
      }
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 p-4">
      <div
        className="relative w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl border max-h-[90vh] flex flex-col"
        ...
        <div className="px-6 py-4 border-b flex items-center justify-between flex-shrink-0" style={{ borderColor: cor.borda }}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: cor.destaque }}>
              <FileStack className="w-5 h-5" style={{ color: cor.fundo }} />
            </div>
            <div>
              <h2 className="text-lg font-bold" style={{ color: cor.tinta }}>Consulta Completa — CPF</h2>
              <p className="text-sm" style={{ color: cor.tintaMuted }}>Dados + Restrições + Protestos</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {step === 'loading' && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p style={{ color: cor.tinta }}>Consultando...</p>
              <p className="text-sm mt-1" style={{ color: cor.tintaMuted }}>Dados, Restrições e Protestos ao mesmo tempo</p>
            </div>
          )}
                ...
                <div className="max-h-[360px] overflow-y-auto">
                  {resultado.map((item, index) => (
                    <div
                      ...
                      style={{ borderColor: cor.borda, backgroundColor: index % 2 === 0 ? cor.faixaAlt : cor.fundo }}
                    >
                      {item.label === '---' ? (
