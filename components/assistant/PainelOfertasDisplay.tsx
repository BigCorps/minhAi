  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        if (!res.ok || json.error) throw new Error(json.error || 'Erro ao carregar imagens');
        if (!json.images || json.images.length === 0) throw new Error('Nenhuma imagem encontrada na pasta.');
    ...
    >
      ...
      {!loading && error && (
        <div className="flex flex-col items-center gap-4 px-8 text-center">
          ...
          <button onClick={handleClose} className="px-6 py-2 bg-orange-600 text-white rounded-full text-sm">
            Fechar
          </button>
        </div>
      )}
