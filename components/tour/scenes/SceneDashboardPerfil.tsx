...
// Perfil do usuário — mesmo padrão visual do SceneDashboardVisao
...
// ─── Dados ────────────────────────────────────────────────────────────────
const MENU_ITEMS = [
  { key: 'perfil',       label: 'Perfil',          Icon: IcoUser,       color: '#3b82f6' },
  { key: 'creditos',     label: 'Créditos',         Icon: IcoCreditCard, color: '#10b981' },
  { key: 'recebimentos', label: 'Recebimentos',     Icon: IcoWallet,     color: '#f59e0b' },
  { key: 'historico',    label: 'Histórico',        Icon: IcoChat,       color: '#8b5cf6' },
  { key: 'indique',      label: 'Indique e Ganhe',  Icon: IcoUsers,      color: '#84cc16' },
  { key: 'ajuda',        label: 'Ajuda',            Icon: IcoLifeBuoy,   color: '#ef4444' },
]
...
const SECTION_DWELL = 4000  // ms em cada seção
...
export default function SceneDashboardPerfil() {
  ...
  // Reset row animation when section changes
  useEffect(() => {
    setRowStep(0)
  }, [activeSection])

  // Animate rows in for sections that have lists
  useEffect(() => {
    const maxRows = activeSection === 'recebimentos' ? HISTORICO_ROWS.length
      : activeSection === 'historico' ? HISTORICO_CHAT.length
      : activeSection === 'ajuda' ? AJUDA_CARDS.length
      : 0
    if (maxRows === 0 || rowStep >= maxRows) return
    const t = setTimeout(() => setRowStep(v => v + 1), 280)
    timers.current.push(t)
    return () => clearTimeout(t)
  }, [activeSection, rowStep])

  // Auto-cycle sections
  useEffect(() => {
    const idx = SECTION_ORDER.indexOf(activeSection)
    const t = add(setTimeout(() => {
      setActiveSection(SECTION_ORDER[(idx + 1) % SECTION_ORDER.length])
    }, SECTION_DWELL))
    return () => clearTimeout(t)
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <div style={{ color: '#fff', fontSize: 13, fontWeight: 700, lineHeight: 1.2 }}>Café Exemplo</div>
                <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 9.5 }}>cafe@exemplo.com.br</div>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div style={{
                ...
                <div style={{ width: 8, height: 8, color: '#34d399' }}><IcoShield /></div>
                BIOMETRIA ATIVA
              </div>
            </div>
          </div>
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: 8.5, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.5px', padding: '0 6px', marginBottom: 4 }}>
                Menu Usuário
              </div>
              {MENU_ITEMS.map(item => {
                ...
                return (
                  ...
                  >
                    <div style={{ width: 13, height: 13, flexShrink: 0, color: active ? item.color : 'rgba(255,255,255,0.45)' }}>
                      <item.Icon />
                    </div>
                    <span style={{
                      fontSize: 10.5,
                ...
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, flex: 1 }}>
                    ...
                    <div style={{ background: 'rgba(30,41,59,0.5)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
                      ...
                      <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: 8.5, marginTop: -4 }}>⚠ A chave Pix não pode ser alterada após o preenchimento.</div>
                    </div>
                  </div>
                </>
              )}
                  ...
                  <div style={{ flex: 1, background: 'rgba(30,41,59,0.5)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                    ...
                    {HISTORICO_ROWS.slice(0, rowStep).map((r, i) => (
                      <div key={i} style={{ display: 'grid', gridTemplateColumns: '110px 60px 1fr 90px 90px', padding: '7px 14px', borderBottom: '0.5px solid rgba(255,255,255,0.04)', alignItems: 'center' }}>
                        ...
                        <div>
                          <span style={{ background: r.tipo === 'SAQUE' ? 'rgba(239,68,68,0.15)' : 'rgba(59,130,246,0.15)', color: r.tipo === 'SAQUE' ? '#f87171' : '#60a5fa', fontSize: 8, fontWeight: 700, borderRadius: 4, padding: '2px 6px' }}>{r.tipo}</span>
                        </div>
                        ...
                        <div>
                          <span style={{ background: r.status === 'CONFIRMADO' ? 'rgba(16,185,129,0.15)' : 'rgba(139,92,246,0.15)', color: r.status === 'CONFIRMADO' ? '#34d399' : '#a78bfa', fontSize: 7.5, fontWeight: 700, borderRadius: 4, padding: '2px 6px' }}>{r.status}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                      <button style={{ background: 'rgba(255,255,255,0.06)', border: '0.5px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.7)', borderRadius: 8, padding: '5px 10px', fontSize: 9, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} style={{ width: 10, height: 10 }}><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3" /></svg>
                        Exportar PDF
                      </button>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6, overflow: 'hidden' }}>
                    {HISTORICO_CHAT.slice(0, rowStep).map((r, i) => (
                      <div key={i} style={{ background: 'rgba(30,41,59,0.5)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={{ width: 20, height: 20, borderRadius: 5, background: 'rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <div style={{ width: 10, height: 10, color: 'rgba(255,255,255,0.4)' }}><IcoUser /></div>
                          </div>
                          <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 6, padding: '5px 10px', flex: 1, color: 'rgba(255,255,255,0.5)', fontSize: 9 }}>{r.msg}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, flex: 1 }}>
                    {AJUDA_CARDS.slice(0, rowStep).map((c, i) => (
                      <div key={i} style={{ background: 'rgba(30,41,59,0.5)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '14px 12px', display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-start' }}>
                        ...
                        <button style={{
                          background: '#b0cb1f', color: '#fff', border: 'none', borderRadius: 8,
                          padding: '5px 10px', fontSize: 9, fontWeight: 700, cursor: 'pointer', marginTop: 'auto',
                        }}>
                          Acessar
                        </button>
                      </div>
