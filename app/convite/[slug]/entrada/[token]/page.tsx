...
export default async function Entrada({ params }: Props) {
  const { slug, token } = await params; const evento = await buscarEventoPublicado(slug); if (!evento) notFound();
  const admin = adminConviteria();
  const [{ data: fam }, { data: pessoa }] = await Promise.all([
    admin.from('convidado_familias').select('id').eq('evento_id', evento.id).eq('qr_token', token).maybeSingle(),
