'use client'
// components/tour/TourStage6.tsx

import { useState, useEffect, useRef, useCallback } from 'react'
import { STAGE6_SCRIPT, Stage6SceneId } from '@/lib/tour/stage6-script'
...
import SceneCobrancaIntro from './scenes/SceneCobrancaIntro'
import SceneCobrancaPix from './scenes/SceneCobrancaPix'
import SceneCobrancaTef from './scenes/SceneCobrancaTef'
import SceneCobrancaNfc from './scenes/SceneCobrancaNfc'
import SceneCobrancaLink from './scenes/SceneCobrancaLink'
import SceneCobrancaRecebimentos from './scenes/SceneCobrancaRecebimentos'
import SceneCobrancaConclusao from './scenes/SceneCobrancaConclusao'
...
export default function TourStage6({
  initialTheme = 'dark',
  onClose,
  onComplete,
  managerDelay = 2000,
  ...
  const playText = useCallback(
    ...
    [_playText, speed] // eslint-disable-line react-hooks/exhaustive-deps
  )
  ...
  const currentScene = STAGE6_SCRIPT[sceneIndex]
  ...
  const runScene = useCallback(async (index: number) => {
    ...
    const scene = STAGE6_SCRIPT[index]
  ...
  const handleNext = useCallback(() => {
    const isLast = sceneIndex === STAGE6_SCRIPT.length - 1
      ...
      >
        ...
        >
          <div className="w-full h-full transition-opacity ease-in-out" style={{ opacity: sceneVisible ? 1 : 0, transitionDuration: `${FADE_DURATION}ms` }}>
            <SceneRenderer6 id={currentScene.id} isSpeaking={isSpeaking} theme={theme} />
          </div>
        </div>
...
function SceneRenderer6({ id, isSpeaking, theme }: { id: Stage6SceneId; isSpeaking: boolean; theme: 'dark' | 'light' }) {
  switch (id) {
    case 'cobranca-intro':        return <SceneCobrancaIntro />
    case 'cobranca-pix':          return <SceneCobrancaPix />
    case 'cobranca-tef':          return <SceneCobrancaTef />
    case 'cobranca-nfc':          return <SceneCobrancaNfc />
    case 'cobranca-link':         return <SceneCobrancaLink />
    case 'cobranca-recebimentos': return <SceneCobrancaRecebimentos />
    case 'cobranca-conclusao':    return <SceneCobrancaConclusao />
    default:                      return null
  }
}
