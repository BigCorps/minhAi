'use client'
// components/tour/TourStage5.tsx

import { useState, useEffect, useRef, useCallback } from 'react'
import { STAGE5_SCRIPT, Stage5SceneId } from '@/lib/tour/stage5-script'
...
import SceneDashboardVisao from './scenes/SceneDashboardVisao'
import SceneDashboardFuncoes from './scenes/SceneDashboardFuncoes'
import SceneDashboardIntegracoes from './scenes/SceneDashboardIntegracoes'
import SceneDashboardGestao from './scenes/SceneDashboardGestao'
import SceneDashboardPerfil from './scenes/SceneDashboardPerfil'
import SceneDashboardConclusao from './scenes/SceneDashboardConclusao'
...
export default function TourStage5({
  initialTheme = 'dark',
  onClose,
  onComplete,
  managerDelay = 2000,
  ...
  const currentScene = STAGE5_SCRIPT[sceneIndex]
  ...
  const runScene = useCallback(async (index: number) => {
    ...
    const scene = STAGE5_SCRIPT[index]
  ...
  const handleNext = useCallback(() => {
    const isLast = sceneIndex === STAGE5_SCRIPT.length - 1
...
function SceneRenderer5({ id, isSpeaking, theme }: { id: Stage5SceneId; isSpeaking: boolean; theme: 'dark' | 'light' }) {
  switch (id) {
    case 'dashboard-visao':        return <SceneDashboardVisao />
    case 'dashboard-funcoes':      return <SceneDashboardFuncoes />
    case 'dashboard-integracoes':  return <SceneDashboardIntegracoes />
    case 'dashboard-gestao':       return <SceneDashboardGestao />
    case 'dashboard-perfil':       return <SceneDashboardPerfil />
    case 'dashboard-conclusao':    return <SceneDashboardConclusao />
    default:                       return null
  }
}
