...
async function enviarConfirmacaoGoogle({
  eventoId, convidadoId, atualizado, idempotencyKey,
}: {
  eventoId: string; convidadoId: string; atualizado: boolean; idempotencyKey: string;
}) {
  ...
  try {
    const r = await fetch(`${base}/functions/v1/conviteia-google-confirmacao`, {
      ...
      body: JSON.stringify({ evento_id: eventoId, convidado_id: convidadoId, atualizado, idempotency_key: idempotencyKey }),
    ...
    console.error('ConviteIA RSVP Google:', error);
    return { emailStatus: 'falhou' as const };
  } finally {
    clearTimeout(timer);
  }
...
export async function POST(req: NextRequest) {
  ...
  const corpo = (await req.json().catch(() => null)) as {
    eventoId?: string; nome?: string; email?: string; acompanhantes?: string[]; solicitacaoId?: string;
  } | null;
  ...
  if (!eventoId || nome.length < 2) return NextResponse.json({ erro: 'Informe seu nome.' }, { status: 400 });
  if (!EMAIL_RE.test(email)) return NextResponse.json({ erro: 'Informe um e-mail valido.' }, { status: 400 });

  const familia = acompanhantes.filter((p) => p.toLocaleLowerCase('pt-BR') !== nome.toLocaleLowerCase('pt-BR'));
  const acesso = await buscarEventoAcessivelPorId(eventoId);
  if (!acesso) return NextResponse.json({ erro: 'Convite indisponivel.' }, { status: 404 });
  ...
  const { data: existente } = await admin.from('convidados')
    .select('id')
  ...
  if (!existente) {
    ...
    if ((count ?? 0) >= MAX_CONFIRMACOES_10_MIN) {
      return NextResponse.json({ erro: 'Muitas confirmacoes deste dispositivo. Aguarde alguns minutos.' }, { status: 429 });
    }
  }
  ...
  if (existente) {
    const r = await admin.from('convidados').update(dados).eq('id', existente.id).select('id').single();
    erro = r.error; convidadoId = r.data?.id ?? null; atualizado = true;
  } else {
    const r = await admin.from('convidados').insert(dados).select('id').single();
    erro = r.error; convidadoId = r.data?.id ?? null;
    if (erro?.code === '23505') {
      const r2 = await admin.from('convidados').update(dados)
        .eq('evento_id', eventoId).eq('email_normalizado', email).select('id').single();
      erro = r2.error; convidadoId = r2.data?.id ?? null; atualizado = !r2.error;
    }
  }

  if (erro || !convidadoId) {
    console.error('ConviteIA RSVP:', erro);
    return NextResponse.json({ erro: 'Nao foi possivel confirmar sua presenca.' }, { status: 500 });
  }

  const agendaUrl = urlGoogleAgenda(evento.config as ConviteConfig, evento.slug as string);

  // O trial não dispara e-mail real pela conta Google do anfitrião. A pessoa
  // consegue testar todo o RSVP e vê o registro imediatamente no dashboard.
  const google = emTeste
    ? { emailStatus: 'sem_google' as const }
    : await enviarConfirmacaoGoogle({ eventoId, convidadoId, atualizado, idempotencyKey: `confirmacao:${solicitacaoId}` });
  ...
  return NextResponse.json({
    ...
    agendaUrl,
    emailStatus: google.emailStatus,
    modoTeste: emTeste,
  });
}
