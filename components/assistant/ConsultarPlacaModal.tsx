...
export default function ConsultarPlacaModal({
  data,
  onClose,
  theme = 'dark',
  playText,
}: ConsultarPlacaModalProps) {
  ...
  const handleConsultar = async () => {
    ...
    try {
      ...
      const params = pendingParams ?? { company_id: companyId, action: 'consultar_placa', placa: placaLimpa };
      ...
      setStep('input');
      playText('Erro ao consultar placa. Tente novamente.').catch(() => {});
    }
  };
  ...
  const handleDownloadPdf = () => {
    ...
    link.href = pdfBase64;
    link.download = pdfFileName || `consulta_placa_${placa.replace(/[^A-Z0-9]/gi, '')}.pdf`;
        ...
        <div className={`px-6 py-4 border-b ${border} ${isDark ? 'bg-yellow-950/40' : 'bg-yellow-50'} flex-shrink-0`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className={`text-xl font-bold ${textPrimary}`}>Consultar Placa</h2>
                <p className={`text-sm ${textMuted}`}>Dados cadastrais do veículo</p>
              </div>
            </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {step === 'loading' && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className={`text-lg font-medium ${textPrimary}`}>Consultando placa...</p>
