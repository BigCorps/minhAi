...
const MCP_FUNCTIONS = [
  { key: 'pix',       label: 'Gerar PIX',        demo: 'pix de 50',             color: '#3B82F6' },
  ...
  { key: 'venda',     label: 'Registrar Venda',   demo: 'venda 100 pix',         color: '#3B82F6' },
  ...
  { key: 'fraude',    label: 'Anti-fraude',       demo: 'fraude https://site.com',color: '#3B82F6' },
...
// ── Carousel de funções ────────────────────────────────────────────────────
function FunctionCarousel({ onSelect, theme }: { onSelect: (demo: string) => void; theme: 'dark' | 'light' }) {
    ...
    <>
      ...
      >
        <div
          ...
          style={{ animation: `mcp-scroll ${count * 2.5}s linear infinite`, willChange: 'transform' }}
        >
          {items.map((fn, idx) => (
            <button
              key={`${fn.key}-${idx}`}
              onClick={() => onSelect(fn.demo)}
...
export default function DashboardMcpWidget({ theme = 'dark' }: { theme?: 'dark' | 'light' }) {
  ...
  const panel = isOpen && (
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className={`text-xs ${isDark ? 'text-white/40' : 'text-gray-500'}`}>Gerencie seu negócio</p>
          </div>
        </div>
        <button
          onClick={() => setIsOpen(false)}
          className={`p-1.5 rounded-lg transition-colors ${isDark ? 'hover:bg-white/10 text-white/60' : 'hover:bg-gray-100 text-gray-500'}`}
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="flex-shrink-0 px-2">
        <FunctionCarousel onSelect={(demo) => send(demo)} theme={theme} />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2 space-y-3">
        {messages.length === 0 && !loading && (
          <div className="flex items-center justify-center h-full">
            <p className={`text-sm text-center ${isDark ? 'text-white/40' : 'text-gray-400'}`}>
              ...
              <span className="text-xs">Clique em uma função acima ou digite um comando</span>
            </p>
          </div>
        )}
        ...
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              className="max-w-[85%] rounded-2xl px-4 py-2.5 text-sm whitespace-pre-wrap shadow"
              style={msg.role === 'user'
        ...
        {loading && (
          <div className="flex justify-start">
            <div
              ...
              style={{ background: isDark ? 'rgba(51,65,85,0.8)' : 'rgba(255,255,255,0.9)' }}
            >
      ...
      >
        ...
        >
          <textarea
            ...
            onKeyDown={handleKey}
            placeholder="Digite um comando... Ex: pix de 50"
            rows={1}
