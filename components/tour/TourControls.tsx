...
export default function TourControls({
  ...
  // Reaparecer quando o tour terminar
  useEffect(() => {
    if (!isPlaying) setHidden(false)
  }, [isPlaying])

  // Ao iniciar o tour, aguarda 1s e some
  const handleToggle = () => {
    if (!isPlaying) {
  ...
  // Hover / touch: mostra e agenda ocultar novamente após 2s
  const handleReveal = () => {
    if (!hidden) return
    setHidden(false)
    if (hideTimerRef.current) clearTimeout(hideTimerRef.current)
    // Se ainda estiver tocando, some de novo após 2s
    if (isPlaying) {
  ...
  return (
    // Zona de detecção maior que o conteúdo visível — captura hover/touch mesmo oculto
    <div
