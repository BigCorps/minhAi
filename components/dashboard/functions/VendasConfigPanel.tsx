  ...
  return (
    <div className="border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
        className="w-full flex items-center justify-between px-5 py-4 bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10 transition text-left"
      >
...
const inputCls =
  'w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-white/10 ' +
...
interface VendasConfigPanelProps {
  companyId: string;
  companyName: string;
}
...
export default function VendasConfigPanel({ companyId, companyName }: VendasConfigPanelProps) {
    ...
    async function load() {
      ...
      try {
        ...
        const { data: company } = await supabase
          .from('companies')
          .select(
            'system_prompt, assistant_role, greeting_message, wake_word, ' +
            'business_address, business_hours, telefone_fixo, whatsapp_number, ' +
            'email_contato, website, receiving_pix_key, receiving_pix_key_type'
          )
          .eq('id', companyId)
          .single();
        ...
        if (company) {
          ...
          setAssistantRole(company.assistant_role || 'Assistente IA');
          setGreetingMessage(company.greeting_message || '');
          setWakeWord(company.wake_word || '');
          setBusinessAddress(company.business_address || '');
          setBusinessHours(company.business_hours || '');
          setTelefoneFixo(company.telefone_fixo || '');
          setWhatsappNumber(company.whatsapp_number || '');
          setEmailContato(company.email_contato || '');
          setWebsite(company.website || '');
          setReceivingPixKey(company.receiving_pix_key || '');
          setReceivingPixKeyType(company.receiving_pix_key_type || '');
        }
        ...
        const { data: settings } = await supabase
          ...
          .in('function_key', VENDAS_FUNCTIONS.map(f => f.key));
  ...
  function toggleFunction(key: string) {
    setEnabledFunctions(prev => ({ ...prev, [key]: !prev[key] }));
  }
    ...
    try {
      ...
      const { error: companyErr } = await supabase
        ...
        .update({
          ...
          website:                website,
          receiving_pix_key:      receivingPixKey || null,
          receiving_pix_key_type: receivingPixKeyType || null,
        })
        .eq('id', companyId);

      if (companyErr) throw companyErr;

      // 2. Salvar estado de cada função
      for (const fn of VENDAS_FUNCTIONS) {
        ...
        if (existing) {
          ...
          await supabase
            .from('company_function_settings')
            .insert({ company_id: companyId, function_key: fn.key, is_enabled });
        }
      }
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      >
        <Field
          ...
          hint="Define como o assistente se apresenta"
        >
          <select
            value={assistantRole}
          ...
          >
            {[
              'Assistente IA', 'Atendente IA', 'Vendedor IA', 'Consultor IA',
              'Operador IA', 'Agente IA', 'Funcionário IA', 'Auxiliar IA',
              'Gerente IA', 'Secretário IA', 'Analista IA', 'Coordenador IA',
        ...
        <Field
          ...
          hint="Ex: olá assistente, oi loja (separe com vírgula para múltiplas)"
        >
          <input
            type="text"
            value={wakeWord}
            onChange={e => setWakeWord(e.target.value)}
            placeholder="olá assistente"
            className={inputCls}
          />
        </Field>
        ...
        <Field
          ...
          hint="Frase falada quando o assistente é ativado"
        >
          <input
            type="text"
        ...
        <Field
          ...
          hint="Descreva o tom, ramo de atuação e instruções especiais. O GROQ usa isso para responder conversas."
        >
          <textarea
            rows={5}
            value={systemPrompt}
      ...
      >
        ...
        <Field label="Horário de Funcionamento">
          <textarea
            ...
            placeholder="Seg–Sex: 08h–18h | Sáb: 08h–12h"
            className={textareaCls}
          />
        </Field>
      ...
      >
        ...
        <Field label="Tipo da Chave PIX">
          <select
            value={receivingPixKeyType}
            onChange={e => setReceivingPixKeyType(e.target.value)}
            className={inputCls}
          >
        ...
        <Field label="Chave PIX" hint="A chave cadastrada no banco para receber pagamentos">
          <input
            type="text"
            value={receivingPixKey}
            onChange={e => setReceivingPixKey(e.target.value)}
      ...
      >
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">
          O token InfinitePay é compartilhado entre NFC Débito, NFC Crédito e Link de Pagamento. Configure uma vez e as três funções ficam prontas.
        </p>
        {/* O InfinitePayConfigForm gerencia seu próprio estado e salva direto via supabase.
            Para o painel Vendas, usamos o modal standalone que já possui load + save interno. */}
      ...
      >
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
          Access Token e Terminal ID são compartilhados entre TEF Débito e TEF Crédito.
        </p>
      </Section>
      ...
      >
        {googleConnected ? (
          <div className="flex items-center gap-3 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
            ...
            <div>
              <p className="text-sm font-medium text-green-800 dark:text-green-200">Google Calendar conectado</p>
              <p className="text-xs text-green-600 dark:text-green-400">Agendamento e consulta de agenda estão disponíveis.</p>
            </div>
          </div>
        ) : (
      ...
      >
        <p className="text-xs text-gray-500 dark:text-gray-400">
          Ative ou desative as funções disponíveis para os clientes deste assistente.
        </p>
        <div className="space-y-2 mt-1">
          {VENDAS_FUNCTIONS.map(fn => {
            const needsGoogle = fn.key === 'agendar_compromisso' || fn.key === 'ver_agenda';
            const isOn = enabledFunctions[fn.key] ?? false;
            return (
              <div
                key={fn.key}
      ...
      {saved && (
        <div className="flex items-center gap-3 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-500/30 rounded-xl">
          <CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0" />
          <p className="text-sm text-green-700 dark:text-green-300">Configurações salvas com sucesso!</p>
        </div>
      )}
...
// ── Subcomponente: campos InfinitePay com load/save próprio ──────────────────
// O InfinitePayConfigForm inline usa `settings` + `onChange` externos mas não
// persiste sozinho. Para o painel Vendas usamos load direto + save no handleSave
// principal. O form é apenas visual — os campos reais são gerenciados aqui.
function InfinitePayStandaloneFields({ companyId }: { companyId: string }) {
  ...
  useEffect(() => {
    supabase
      ...
      .eq('id', companyId)
      .single()
      .then(({ data }) => { if (data) setHandle(data.infinitepay_handle || ''); });
  }, [companyId]);
  ...
  async function save() {
    ...
    await supabase
      .from('companies')
      .update({ infinitepay_handle: handle.trim() })
      .eq('id', companyId);
    setSaving(false);
    setSaved(true);
    ...
    <div className="space-y-3 pt-2 border-t border-gray-100 dark:border-white/10">
      <div>
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            <input
              ...
              className="w-full px-3 py-2.5 pr-10 text-sm rounded-lg border border-gray-300 dark:border-white/10 bg-white dark:bg-slate-800 text-gray-900 dark:text-white font-mono focus:ring-2 focus:ring-amber-500 focus:border-transparent"
            />
            <button
              type="button"
              onClick={() => setShow(v => !v)}
        ...
        <p className="text-xs text-gray-400 mt-1">
          Painel InfinitePay → Integrações → Token. Formato: <code className="bg-gray-100 dark:bg-slate-700 px-1 rounded">$meu-handle</code>
        </p>
      </div>
    </div>
  );
