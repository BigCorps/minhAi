...
type QrBrand = 'pixwiki' | 'minhai' | null

function getPlatformLogoBuffer(brand: Exclude<QrBrand, null>): Buffer {
  if (brand === 'pixwiki') {
    const pixWikiPath = path.join(process.cwd(), 'public', 'brands', 'pix', 'pixwiki.png')
    if (fs.existsSync(pixWikiPath)) return fs.readFileSync(pixWikiPath)
  }

  const minhaiPath = path.join(process.cwd(), 'public', 'icon192.png')
  return fs.readFileSync(minhaiPath)
}
...
async function getLogoBuffer(companyId: string | null, overrideUrl?: string | null, brand: QrBrand = null): Promise<Buffer | null> {
  try {
    // Quando o chamador informa a marca do produto, ela tem prioridade.
    // Chamadas antigas sem `brand` continuam exatamente com a lógica anterior.
    if (brand) {
      return await sharp(getPlatformLogoBuffer(brand)).png().toBuffer()
    }
...
export async function GET(req: NextRequest) {
  ...
  const brand: QrBrand = brandRaw === 'pixwiki' ? 'pixwiki' : brandRaw === 'minhai' ? 'minhai' : null
  ...
  try {
    ...
    const logoBuffer = noLogo ? null : await getLogoBuffer(companyId, logoUrl, brand)
