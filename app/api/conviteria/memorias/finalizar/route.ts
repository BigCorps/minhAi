...
async function excluirReserva(memoria: { id: string; storage_path: string }) {
  ...
  await admin.from('evento_memorias').update({
    status: 'excluido', reserva_expira_em: null, updated_at: new Date().toISOString(),
  }).eq('id', memoria.id).eq('status', 'reservado');
}

export async function POST(req: NextRequest) {
  const corpo = (await req.json().catch(() => null)) as { slug?: string; memoriaId?: string } | null;
  if (!corpo?.slug || !corpo.memoriaId) return NextResponse.json({ erro: 'Envio inválido.' }, { status: 400 });
  ...
  const { data: memoria } = await admin.from('evento_memorias')
    ...
    .eq('id', corpo.memoriaId).eq('evento_id', evento.id).maybeSingle();

  if (!memoria || memoria.status !== 'reservado') return NextResponse.json({ erro: 'Reserva de envio não encontrada.' }, { status: 404 });
  ...
  const pacoteAtivo = Boolean(
    pacote &&
    (pacote.status === 'ativo' || (evento.modoTeste && ['teste', 'aguardando_pagamento'].includes(String(pacote.status)))) &&
  ...
  const { data: objetos, error: listError } = await adminPublic().storage
    .from(MEMORIAS_BUCKET).list(pasta, { search: nome, limit: 5 });
  const objeto = (objetos ?? []).find((o) => o.name === nome && o.id != null);
  if (listError || !objeto) return NextResponse.json({ erro: 'O arquivo ainda não terminou de enviar.' }, { status: 409 });
  ...
  const limiteTipo = memoria.tipo === 'video' ? Number(pacote.video_max_bytes) : Number(pacote.limite_bytes);

  const arquivoInvalido =
    !Number.isFinite(tamanhoReal) || tamanhoReal <= 0 ||
    tamanhoReal > tamanhoReservado || tamanhoReal > limiteTipo ||
  ...
  return NextResponse.json({ ok: true, status: novoStatus, modoTeste: evento.modoTeste });
}
...
export async function DELETE(req: NextRequest) {
  ...
  const admin = adminConviteria();
  const { data: memoria } = await admin.from('evento_memorias')
    .select('id,storage_path,status').eq('id', corpo.memoriaId).eq('evento_id', evento.id).maybeSingle();
  if (memoria?.status === 'reservado') await excluirReserva(memoria as { id: string; storage_path: string });
