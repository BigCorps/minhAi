...
export default function ContratoEmTextoDisplay({ data, onClose, theme = 'dark' }: Props) {
  ...
  const handleReset = () => { setStage('capturing'); setContratoText(null); setErrorMsg(null); setCopied(false); };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 p-4">
      <div className={`w-full max-w-lg rounded-2xl p-6 shadow-2xl ${isDark ? 'bg-slate-800 border border-white/10' : 'bg-white border border-gray-200'}`}>
        ...
        {stage === 'capturing' && (
          <CameraCapture onCapture={handleCapture} onCancel={handleManualClose} theme={theme} companyId={data.companyId} instructions="Fotografe o contrato ou documento legal." />
        )}
        {stage === 'processing' && (
        ...
        {stage === 'result' && contratoText && (
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={async () => { await navigator.clipboard.writeText(contratoText); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
