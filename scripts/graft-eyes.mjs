...
const WINDOW = { left: 360, top: 210, width: 350, height: 200 };
...
for (const file of files) {
  ...
  /**
   * Escala pela largura da cabeca, medida logo abaixo do topo da silhueta.
   *
   * Os frames de olho vem de geracoes proprias e nao saem no mesmo tamanho: o
   * de olhos fechados tinha cabeca de 372 pixels contra 357 da neutra, 4,2% a
   * mais. Enxertado em 1:1, o olho fechado ficava maior que o aberto — e como a
   * piscada alterna entre os dois, o efeito era um olho mudando de tamanho a
   * cada vez.
   *
   * A cabeca e usada em vez da distancia entre corneas porque o frame de olho
   * fechado nao tem cornea nenhuma para medir.
   */
  ...
  for (const cx of [baseEyes.left, baseEyes.right]) {
    const ex = cx - WINDOW.left;
    const ey = baseEyes.cy - WINDOW.top;
    const rx = Math.round(baseEyes.spread * 0.46);
    const ry = Math.round(baseEyes.spread * 0.34);
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        const nx = (x - ex) / rx;
        const ny = (y - ey) / ry;
