...
export default function FAQSection({ theme = 'dark' }: FAQSectionProps) {
  ...
  return (
    <div className={`relative flex flex-col items-center justify-center h-full w-full px-6 sm:px-8 lg:px-12 overflow-hidden transition-colors duration-500 ${
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto flex flex-col gap-5 sm:gap-6">
        ...
        <div className="text-center">
          ...
          <h2
            ...
            className={`text-2xl sm:text-3xl md:text-4xl font-bold leading-tight mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}
          >
        ...
        <div className="flex flex-col gap-2 sm:gap-3 w-full">
          {FAQS.map(({ pergunta, resposta }, i) => (
            <details
              key={i}
              className={`group rounded-xl border px-5 py-4 cursor-pointer transition-all duration-200 ${
                isDark
            ...
            >
              <summary className="flex items-center justify-between gap-3 list-none">
                <h3 className={`font-semibold text-sm sm:text-base pr-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {pergunta}
                </h3>
                <svg
                  className={`w-4 h-4 flex-shrink-0 transition-transform duration-200 group-open:rotate-180 ${isDark ? 'text-white/30' : 'text-gray-400'}`}
                  fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"
                >
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </summary>
              <p className={`mt-3 text-xs sm:text-sm leading-relaxed ${isDark ? 'text-white/50' : 'text-gray-500'}`}>
                {resposta}
              </p>
            </details>
          ))}
        ...
        {/* CTA inline */}
        <div className="text-center">
          <p className={`text-sm mb-3 ${isDark ? 'text-white/35' : 'text-gray-400'}`}>
            Ainda tem dúvidas?
          </p>
          <a
            ...
            className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full border text-sm font-semibold transition-all hover:scale-105 ${
              isDark
