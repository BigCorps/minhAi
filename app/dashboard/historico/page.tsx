...
export default function HistoricoPage() {
  ...
  useEffect(() => {
    if (typeDropdownOpen && typeButtonRef.current) {
      ...
      setTypeDropdownPos({ top: r.bottom + window.scrollY, left: r.left + window.scrollX, width: Math.max(r.width, 140) });
    }
  }, [typeDropdownOpen]);
  ...
  useEffect(() => {
    function onMouseDown(e: MouseEvent) {
      ...
      if (typeDropdownRef.current && !typeDropdownRef.current.contains(e.target as Node) &&
          typeButtonRef.current && !typeButtonRef.current.contains(e.target as Node)) {
  ...
  async function enrichWithMessages(batch: LogEntry[]): Promise<LogEntry[]> {
    ...
    let result = batch.map(log => {
      ...
      if (META_FUNCTIONS.includes(log.function_key) && log.metadata) {
        const u = log.metadata.from_message || log.metadata.comment_text || log.metadata.message || '';
        const a = log.metadata.reply_text || log.metadata.response || '';
