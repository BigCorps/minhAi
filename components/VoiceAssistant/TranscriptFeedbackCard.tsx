...
/**
 * Componente: TranscriptFeedbackCard
 * 
 * Exibe o último texto reconhecido pelo Google Speech em tempo real.
 * Ajuda o cliente a entender:
 * - O que o assistente entendeu
 * - Por que não respondeu (se foi um erro de reconhecimento)
 * - Se precisa falar mais pausadamente
 * 
 * Aparece acima do TextInputChat e desaparece após 5 segundos de inatividade.
 */
...
export function TranscriptFeedbackCard({
  ...
  // Mostrar card quando houver transcript ou resposta
  useEffect(() => {
      ...
      >
        ...
        {hasResponse && (
          <div className="space-y-1">
            ...
            <p
              className={`text-sm px-3 py-2 rounded-lg line-clamp-2 ${
                isDark
        ...
        {hasTranscript && !hasResponse && (
          <div className="flex items-start gap-2 p-2 rounded-lg bg-amber-900/20 border border-amber-700/30">
            ...
            <p className="text-xs text-amber-300">
              Nenhuma função foi ativada. Tente falar a palavra de ativação seguida de um comando.
            </p>
          </div>
        )}
        ...
        {isListening && (
          ...
          >
            Fale pausadamente para melhor reconhecimento
          </p>
        )}
      </div>
    </div>
