...
import { Check, Download, ExternalLink, Film, Image as ImageIcon, Loader2, QrCode, Trash2, X, Eye, EyeOff } from 'lucide-react';
...
export default function MemoriasPainel({ eventoId, titulo }: { eventoId: string; slug: string; titulo: string }) {
  ...
  async function baixarTudo() {
    ...
    try {
      // Renova as URLs assinadas imediatamente antes do ZIP. Assim o download
      // funciona mesmo se o painel ficou aberto por mais de uma hora.
      ...
      for (const m of frescos.midias.filter((x) => x.status !== 'excluido' && x.status !== 'reservado')) {
        ...
        if (m.tipo === 'foto') fotos.file(`foto-${String(++fi).padStart(3, '0')}.${ext(m.mimeType, m.tipo)}`, blob);
        else videos.file(`video-${String(++vi).padStart(3, '0')}.${ext(m.mimeType, m.tipo)}`, blob);
      }
    ...
    <section className="mt-3 border-t pt-3" style={{ borderColor: '#c0607822' }}>
      ...
      {!aberto ? null : carregando && !dados ? <div className="flex justify-center py-6"><Loader2 className="h-5 w-5 animate-spin" style={{ color:'#c06078' }} /></div> : erro && !dados ? <p className="p-3 text-sm text-red-700">{erro}</p> : dados && !dados.ativo ? (
        ...
        <div className="mt-3 space-y-3">
          ...
          {dados.midias.length > 0 && <div className="rounded-2xl border bg-white p-4" style={{ borderColor:'#c0607830' }}><div className="mb-3 flex items-center justify-between"><p className="font-semibold" style={{ color:'#40232c' }}>Arquivos recebidos</p><button onClick={() => void carregar()} className="text-xs" style={{ color:'#a04a63' }}>Atualizar</button></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {dados.midias.filter((m) => m.status !== 'excluido' && m.status !== 'reservado').map((m) => <div key={m.id} className="overflow-hidden rounded-xl border" style={{ borderColor:'#c0607828' }}><div className="aspect-square bg-black/5">{m.tipo === 'foto' ? <img src={m.url} alt="" className="h-full w-full object-cover" /> : <video src={m.url} className="h-full w-full object-cover" preload="metadata" />}</div><div className="p-2"><div className="flex items-center justify-between gap-1"><span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${m.status === 'aprovado' ? 'bg-emerald-50 text-emerald-700' : m.status === 'pendente' ? 'bg-amber-50 text-amber-700' : 'bg-gray-100 text-gray-600'}`}>{m.status}</span><span className="text-[10px] text-gray-400">{mb(m.tamanhoBytes)}</span></div><div className="mt-2 flex gap-1">{m.status !== 'aprovado' && <button title="Aprovar" onClick={() => void patch({ memoriaId:m.id,status:'aprovado' }).catch((x) => setErro(x.message))} className="rounded-lg bg-emerald-50 p-1.5 text-emerald-700"><Check className="h-3.5 w-3.5" /></button>}{m.status === 'aprovado' && <button title="Ocultar" onClick={() => void patch({ memoriaId:m.id,status:'oculto' }).catch((x) => setErro(x.message))} className="rounded-lg bg-gray-100 p-1.5 text-gray-600"><EyeOff className="h-3.5 w-3.5" /></button>}{m.status === 'oculto' && <button title="Mostrar" onClick={() => void patch({ memoriaId:m.id,status:'aprovado' }).catch((x) => setErro(x.message))} className="rounded-lg bg-emerald-50 p-1.5 text-emerald-700"><Eye className="h-3.5 w-3.5" /></button>}<button title="Excluir" onClick={() => void excluir(m.id)} className="rounded-lg bg-red-50 p-1.5 text-red-700"><Trash2 className="h-3.5 w-3.5" /></button></div></div></div>)}
          </div></div>}
        </div>
      ) : null}
    </section>
