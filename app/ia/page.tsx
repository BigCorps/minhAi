...
export default function IARootPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (slug.trim()) {
      router.push(`/ia/${slug.trim().toLowerCase()}`);
    }
  };
      ...
      <div style={{ width: '100%', maxWidth: '384px' }}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <div style={{ width: '80px', height: '80px', borderRadius: '16px', background: '#2563eb', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
            <span style={{ color: '#fff', fontSize: '32px', fontWeight: 700 }}>AI</span>
          </div>
          <h1 style={{ fontSize: '24px', fontWeight: 700, color: textPrimary, margin: '0 0 8px' }}>Acessar Assistente</h1>
          <p style={{ color: textSecondary, fontSize: '14px', margin: 0 }}>Digite o identificador do assistente</p>
        </div>
