...
export default function LerCodigoBarrasDisplay({ data, onClose, theme = 'dark', playText }: Props) {
  ...
  useEffect(() => { setHasBarcodeDetector('BarcodeDetector' in window); }, []);
  ...
  // Scan automático via BarcodeDetector
  useEffect(() => {
  ...
  useModalVoiceCommand({
    ...
    onTranscript: (transcript) => {
      ...
      if (['repetir', 'repete', 'de novo', 'nao ouvi'].some(c => t.includes(c))) {
        playText(stage === 'result' && result ? `Código lido: ${result}` : OPENING_TEXT).catch(() => {}); return;
      }
      if (stage === 'capturing') {
        ...
        {stage === 'result' && result && (
          <div className="flex flex-col gap-4">
            ...
            <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium ${isDark ? 'bg-green-900/30 border border-green-700 text-green-300' : 'bg-green-50 border border-green-200 text-green-700'}`}>
              <Check className="w-4 h-4 shrink-0" />Código lido com sucesso!
            </div>
            ...
            {resultQrUrl && (
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="p-2 bg-white rounded-xl shadow-sm">
                  ...
                  <img src={resultQrUrl} alt="QR Code do código de barras" width={140} height={140} className="rounded-lg" />
                </div>
              </div>
            )}
