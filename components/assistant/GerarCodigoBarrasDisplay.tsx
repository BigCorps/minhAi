...
import { X, UtensilsCrossed, IconBarcode, Copy, Check, ExternalLink, QrCode } from 'lucide-react';
...
const FORMAT_VOICE_MAP: Record<string, Format> = {
  ...
  'ean': 'EAN13', 'ean 13': 'EAN13', 'ean13': 'EAN13', 'treze': 'EAN13', 'ean treze': 'EAN13',
  ...
  const validateForFormat = (text: string, fmt: Format): string | null => {
    ...
    if (fmt === 'EAN13') {
      ...
      if (digits.length !== 12 && digits.length !== 13)
        return 'EAN-13 requer exatamente 12 dígitos numéricos (o 13º é calculado automaticamente).';
    }
    if (fmt === 'CODE39') {
  ...
  useModalVoiceCommand({
    ...
    onTranscript: (transcript) => {
      ...
      if (stage === 'result') {
        ...
        if (['novo', 'gerar outro', 'nova consulta', 'tentar novamente'].some(c => t.includes(c))) {
          handleReset(); return;
        }
        return;
      }
      ...
      // Input
      if (stage === 'input') {
        if (['gerar', 'criar', 'confirmar', 'confirma', 'ok', 'gerar codigo', 'criar codigo'].some(c => t.includes(c))) {
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            <IconBarcode className="w-5 h-5" style={{ color: '#3b82f6' } as React.CSSProperties} />
        ...
        {stage === 'input' && (
          <div className="flex flex-col gap-4">
            ...
            <VoiceHint
              commands={['"gerar"', '"code 128"', '"ean 13"', '"code 39"', '"fechar"']}
              isDark={isDark}
            />
          </div>
        )}
