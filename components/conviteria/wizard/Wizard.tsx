...
import Traje from './etapas/Traje';
...
export default function Wizard({
  ...
  const [estado, despachar] = useReducer(
    ...
    (inicial: EstadoWizard | undefined) =>
      normalizarEstadoWizard(inicial ?? criarEstadoInicial()),
  );
  ...
  useEffect(() => {
    ...
    const esquerda = Math.max(0, atual.offsetLeft - (lista.clientWidth - atual.offsetWidth) / 2);
    ...
    <div className="wz">
      ...
      <div className="wz-painel" ref={painel}>
        ...
        <header className="wz-cabecalho">
          <ol className="wz-trilha" ref={trilha}>
            {etapas.map((e, i) => (
              <li
                key={e.id}
                className={i === estado.etapa ? 'atual' : i < estado.etapa ? 'feita' : ''}
              >
                <button
                  type="button"
        ...
        <footer className="wz-rodape">
          ...
          >
            Ver prévia <span className="wz-badge">{estado.etapa + 1}</span>
          </button>
          ...
          <button
            type="button"
            className={`wz-btn wz-btn-principal${concluindo ? ' wz-btn-carregando' : ''}`}
          ...
          >
            {ultima && concluindo ? (
              <>
                <Loader2 className="wz-btn-spinner" aria-hidden="true" />
                {modo === 'editar' ? 'Salvando alterações…' : 'Processando…'}
              </>
            ) : ultima ? (
              modo === 'editar' ? 'Salvar alterações' : 'Ir para o pagamento'
            ) : 'Continuar'}
          </button>
        </footer>
      </div>
      ...
      <aside className={`wz-previa${previaAberta ? ' aberta' : ''}`}>
        <div className="wz-previa-topo">
          <span>Prévia</span>
          <button type="button" className="wz-fechar" onClick={() => setPreviaAberta(false)} aria-label="Fechar prévia">✕</button>
        </div>
        ...
        <div className="wz-previa-palco">
          {abaPrevia === 'envelope' ? (
            <div className="wz-envelope-palco" style={tokensDoConvite(estado.cfg.temaId, estado.cfg.fonteId)}>
              <Capa
