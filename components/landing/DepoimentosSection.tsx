    ...
    >
      ...
      >
        ...
        <div className="text-center">
          <p className={`text-[10px] sm:text-xs font-semibold uppercase tracking-widest mb-1 sm:mb-2 ${isDark ? 'text-green-400/70' : 'text-green-600/70'}`}>
            O que dizem os clientes
          </p>
          <h2
            className={`
              font-bold leading-tight mb-0.5 sm:mb-1
              text-lg
              [@media(min-height:700px)_and_(max-width:767px)]:text-xl
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4 w-full">
          {DEPOIMENTOS.map((d) => {
            ...
            return (
              <article
                key={d.nome}
                className={`relative flex flex-col gap-2 sm:gap-3 p-3.5 sm:p-5 rounded-2xl border ${c.border} ${c.cardBg}`}
                itemScope
                itemType="https://schema.org/Review"
              >
                {/* Aspas decorativas */}
                <svg
                  className={`absolute top-3 right-3 w-5 h-5 sm:w-8 sm:h-8 ${c.quote}`}
                  aria-hidden="true"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <div className="flex gap-0.5" aria-label={`${d.estrelas} estrelas`}>
                  {STARS.map((_, i) => (
                    <svg key={i} className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                ...
                {/* Badge resultado — some em telas muito baixas */}
                <div
                  className={`
                    inline-flex items-center gap-1 sm:gap-1.5
                    px-2 py-0.5 sm:px-3 sm:py-1.5
                    rounded-full text-[9px] sm:text-[11px] font-semibold border self-start
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div itemProp="author" itemScope itemType="https://schema.org/Person">
                    ...
                    <p className={`text-[9px] sm:text-[11px] ${isDark ? 'text-white/40' : 'text-gray-400'}`}>
                      {d.cargo} · {d.empresa}
                    </p>
                  </div>
                  <span className={`ml-auto text-[9px] sm:text-xs ${isDark ? 'text-white/30' : 'text-gray-400'}`}>
                    {d.segmento}
                  </span>
                </div>
              </article>
        ...
        {/* Trust signals — somem em telas baixas */}
        <div
          className={`
