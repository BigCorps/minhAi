...
// components/consultatec/ConsultarProtestosModal.tsx — mesmo padrão, ação consultar_protestos.
// Só CPF: a ferramenta original (ferramentas-consultas) não tem rota de protestos para CNPJ.

import { useState } from 'react';
import { createPortal } from 'react-dom';
import { X, ShieldCheck, Loader2, AlertCircle, CheckCircle2, Download } from 'lucide-react';
...
export default function ConsultarProtestosModal({ data, onClose }: Props) {
  ...
  const handleConsultar = async () => {
    ...
    try {
      ...
      const params = pendingParams ?? { company_id: companyId, action: 'consultar_protestos', cpf: cpfLimpo };

      const { data: res, error: fnError } = await supabase.functions.invoke('ferramentas-consultas', { body: params });
      if (fnError) throw new Error(fnError.message);

      if (res.requires_payment) {
        setPendingParams({ company_id: companyId, action: 'consultar_protestos', cpf: cpfLimpo, payment_confirmed: true, idempotency_key: res.idempotency_key });
        setStep('input');

        const pixRes = await supabase.functions.invoke('gerar-pix-assistente', {
          body: { company_id: companyId, amount_cents: res.amount_cents, purpose: 'consulta_fee', description: `Protestos CPF - R$ ${res.amount_brl}`, brand: 'consultatec' },
        });
        ...
        setPixInfo({ qrCodeUrl: pixRes.data.qr_code_url, pixCode: pixRes.data.pix_code, amountBrl: res.amount_brl ?? '10,00' });
        return;
      }
      ...
      setResultado(rows);
      setPdfFileName(`protestos-cpf-${cpfLimpo}.pdf`);
      setPdfBase64(generateConsultaPDF('Protestos CPF', res.resultado_formatado || []));
      setPendingParams(null);
      setPixInfo(null);
      setStep('result');
    } catch (err: any) {
      setError(err.message || 'Erro ao consultar protestos. Tente novamente.');
      setStep('input');
    }
  };
        ...
        <div className="px-6 py-4 border-b flex items-center justify-between flex-shrink-0" style={{ borderColor: cor.borda }}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: cor.destaque }}>
              <ShieldCheck className="w-5 h-5" style={{ color: cor.fundo }} />
            </div>
            <div>
              <h2 className="text-lg font-bold" style={{ color: cor.tinta }}>Protestos</h2>
              <p className="text-sm" style={{ color: cor.tintaMuted }}>Protestos em cartório e pendências tributárias</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {step === 'loading' && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p style={{ color: cor.tinta }}>Consultando protestos...</p>
            </div>
          )}
