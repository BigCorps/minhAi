...
export default function ImpressaoRemotaDisplay({ data, onClose, theme = 'dark', playText }: Props) {
  ...
  const handleCapture = useCallback(async (base64: string) => {
    ...
    try {
      ...
      let fileName = `impressao_remota_${Date.now()}.jpg`;
      let estimatedPages = 1;

      if (base64.startsWith('JVBERi')) {
        fileType = 'application/pdf';
        fileName = `impressao_remota_${Date.now()}.pdf`;
        ...
        {stage === 'upload' && (
          <div className="flex flex-col gap-3">
            ...
            <CameraCapture
              ...
              theme={theme}
              companyId={data.companyId}
              instructions="Envie um arquivo ou tire uma foto para impressão remota."
              acceptPdf={true}
              activeTab={cameraTab}
