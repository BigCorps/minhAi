...
interface TourControlsProps {
  ...
  /**
   * Quando true, os controles ficam posicionados como overlay absoluto
   * no centro-baixo da cena, revelados por hover/click no container pai.
   * O container pai deve ter position: relative e chamar onReveal.
   * Usado no modal desktop onde os controles ficam sobre a cena.
   */
  overlayMode?: boolean
  /** Forçar visibilidade (usado pelo container pai no overlayMode) */
...
export default function TourControls({
  ...
  // No overlayMode, forceVisible controla a visibilidade
  // No modo normal, a lógica de hide/show local continua igual
  ...
  const handleReveal = () => {
    if (overlayMode) return // overlayMode é controlado pelo pai
    if (!hidden) return
    setHidden(false)
  ...
  // overlayMode: posicionado absolutamente sobre a cena, centro-baixo
  if (overlayMode) {
    return (
      <div
  ...
  // Modo normal: rodapé com zona de detecção de hover
  return (
    <div
