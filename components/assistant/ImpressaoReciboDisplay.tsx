...
export default function ImpressaoReciboDisplay({ data, onClose, theme = 'dark', playText }: Props) {
  ...
  const handleCapture = useCallback(async (base64: string) => {
    ...
    try {
      ...
      if (chargeEnabled) {
        ...
        setStage('printing');
        // Passa paymentMethod e base64 diretamente — não depende do state
        await processPrint(job.id, base64, 'credits');
      }
  ...
  const processPrint = async (jobId: string, contentBase64: string, paymentMethod: 'pix' | 'credits') => {
    try {
